...
CHE_IMAGE_TAG - Set che-server image tag, defaults to "7.0.0-beta-4.0"
...
DEFAULT_CHE_IMAGE_TAG="7.0.0-beta-4.0"
...
DEFAULT_KEYCLOAK_IMAGE_TAG="7.0.0-beta-4.0"
...
DEFAULT_PLUGIN__REGISTRY__URL="https://che-plugin-registry.openshift.io/v2"
...
${CHE_VAR_ARRAY}"
    ...
    if [ "${DEPLOY_CHE_PLUGIN_REGISTRY}" == "true" ]; then
        PLUGIN_REGISTRY_ROUTE=$($OC_BINARY get route/che-plugin-registry --namespace=${CHE_OPENSHIFT_PROJECT} -o=jsonpath={'.spec.host'})
        PLUGIN__REGISTRY__URL="${HTTP_PROTOCOL}://${PLUGIN_REGISTRY_ROUTE}/v2"
    fi
