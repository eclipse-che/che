 ...
 *******************************************************************************/
package org.eclipse.che.plugin.git.ide.pull;
...
import org.eclipse.che.plugin.git.ide.BranchSearcher;
...
import org.eclipse.che.plugin.git.ide.compare.branchList.BranchListPresenter;
import org.eclipse.che.plugin.git.ide.remote.RemotePresenter;
...
public class PullPresenter implements PullView.ActionDelegate {
        ...
        service.remoteList(appContext.getDevMachine(), project.getLocation(), null, true)
               ...
               .catchError(new Operation<PromiseError>() {
                   ...
                   public void apply(PromiseError error) throws OperationException {
                       handleError(error.getCause(), RemotePresenter.REMOTE_REPO_COMMAND_NAME);
                       view.setEnablePullButton(false);
                   }
               });
    }
    ...
    private void updateBranches(@NotNull final BranchListMode remoteMode) {
        ...
        service.branchList(appContext.getDevMachine(), project.getLocation(), remoteMode).then(new Operation<List<Branch>>() {
            ...
            @Override
            public void apply(PromiseError error) throws OperationException {
                handleError(error.getCause(), BranchListPresenter.BRANCH_LIST_COMMAND_NAME);
                view.setEnablePullButton(false);
            }
        });
    }
    ...
    private void handleError(@NotNull Throwable exception, @NotNull String commandName) {
        ...
        if (errorMessage == null) {
            switch (commandName) {
                case RemotePresenter.REMOTE_REPO_COMMAND_NAME:
                    errorMessage = constant.remoteListFailed();
                    break;
                case BranchListPresenter.BRANCH_LIST_COMMAND_NAME:
