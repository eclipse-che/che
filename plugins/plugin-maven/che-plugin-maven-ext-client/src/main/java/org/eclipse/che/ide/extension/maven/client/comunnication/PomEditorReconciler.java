...
import org.eclipse.che.ide.extension.maven.client.editor.PomReconsilingStrategy;
                ...
                for (EditorPartPresenter openedEditor : openedEditors) {
                    ...
                    if (pomPaths.contains(path)) {
                        if (openedEditor instanceof ConfigurableTextEditor) {
                            final Reconciler reconciler = ((ConfigurableTextEditor)openedEditor).getConfiguration().getReconciler();
                            if (reconciler != null) {
