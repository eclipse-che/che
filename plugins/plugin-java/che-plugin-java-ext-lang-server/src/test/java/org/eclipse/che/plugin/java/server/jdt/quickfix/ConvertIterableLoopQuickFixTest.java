...
public final class ConvertIterableLoopQuickFixTest extends QuickFixTest {
  ...
  private List fetchConvertingProposal(StringBuffer buf, ICompilationUnit cu) throws Exception {
    ...
    fConvertLoopProposal =
        (FixCorrectionProposal)
            findProposalByCommandId(QuickAssistProcessor.CONVERT_FOR_LOOP_ID, proposals);
    return proposals;
  }
  ...
  public void testCorrectIteratorMethod() throws Exception {
    ...
    assertTrue(
        fConvertLoopProposal.getFixStatus() != null && fConvertLoopProposal.getFixStatus().isOK());
