...
public class CheASTParser {
	...
	private List getClasspath() throws IllegalStateException {
		Main main =
				new Main(new PrintWriter(System.out), new PrintWriter(System.err), false/*systemExit*/, null/*options*/, null/*progress*/);
	 ...
	 * explicitly specified for the {@link CheASTParser} instance.
	...
	public void createASTs(ICompilationUnit[] compilationUnits, String[] bindingKeys, ASTRequestor requestor, IProgressMonitor monitor) {
		try {
			...
			if ((this.bits & CompilationUnitResolver.RESOLVE_BINDING) != 0) {
				...
				CompilationUnitResolver.resolve(compilationUnits, bindingKeys, requestor, this.apiLevel, this.compilerOptions, this.project,
	 ...
	 * <ul>
	 * <li>{@link CheASTParser#createAST(IProgressMonitor) ASTParser.createAST} is called to parse it
	 * and create a corresponding AST. The calls to
	 * {@link CheASTParser#createAST(IProgressMonitor) ASTParser.createAST} all employ the same settings.</li>
		...
		try {
			...
			if ((this.bits & CompilationUnitResolver.RESOLVE_BINDING) != 0) {
				...
				CompilationUnitResolver
						.resolve(sourceFilePaths, encodings, bindingKeys, requestor, this.apiLevel, this.compilerOptions, getClasspath(),
								 flags, monitor);
			} else {
	...
	public IBinding[] createBindings(IJavaElement[] elements, IProgressMonitor monitor) {
		try {
			...
			return CompilationUnitResolver
					.resolve(elements, this.apiLevel, this.compilerOptions, this.project, this.workingCopyOwner, flags, monitor);
		} finally {
			// reset to defaults to allow reuse (and avoid leaking)
			initializeDefaults();
		}
	...
	private ASTNode internalCreateASTForKind() {
		...
		CodeSnippetParsingUtil
				codeSnippetParsingUtil = new CodeSnippetParsingUtil((this.bits & CompilationUnitResolver.IGNORE_METHOD_BODIES) != 0);
