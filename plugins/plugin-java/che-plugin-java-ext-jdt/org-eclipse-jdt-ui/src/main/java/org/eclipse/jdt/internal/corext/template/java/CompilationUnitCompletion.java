...
final class CompilationUnitCompletion extends CompletionRequestor {
	...
	public final class Variable {
		...
		public String[] getMemberTypeSignatures() {
			if (isArray()) {
				return new String[] {
						Signature.createArraySignature(Signature.getElementType(signature), Signature.getArrayCount(signature) - 1)};
		...
		public String[] computeBinding(String superType, int index) throws JavaModelException, IndexOutOfBoundsException {
			...
			if (type == null)
				throw new JavaModelException(new CoreException(
						new Status(IStatus.ERROR, JavaPlugin.getPluginId(), IStatus.OK, "No such type", null))); //$NON-NLS-1$
