	...
	public RefactoringStatus checkInitialConditions(IProgressMonitor pm) throws CoreException {
		...
		if (!Checks.isDeclaredIn(fTempDeclarationNode, MethodDeclaration.class))
			return RefactoringStatus.createFatalErrorStatus(RefactoringCoreMessages
																	.PromoteTempToFieldRefactoring_only_declared_in_methods);
	...
	private Javadoc getNewConstructorComment(ASTRewrite rewrite) throws CoreException {
		if (StubUtility.doAddComments(fCu.getJavaProject())){
			String comment= CodeGeneration
					.getMethodComment(fCu, getEnclosingTypeName(), getEnclosingTypeName(), new String[0], new String[0], null, null,
	...
	private RefactoringStatus initialize(JavaRefactoringArguments arguments) {
		...
		if (initialize != null && !"".equals(initialize)) {//$NON-NLS-1$
			...
			try {
				...
				return RefactoringStatus.createFatalErrorStatus(
						Messages.format(RefactoringCoreMessages.InitializableRefactoring_argument_not_exist, ATTRIBUTE_INITIALIZE));
			}
			fInitializeIn = value;
		}
