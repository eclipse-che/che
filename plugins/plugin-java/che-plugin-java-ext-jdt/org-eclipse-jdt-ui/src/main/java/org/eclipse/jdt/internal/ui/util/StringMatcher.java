...
public class StringMatcher {
   ...
   *
   * <p>Literal '*' and '?' characters must be escaped in the pattern e.g., "\*" means literal "*",
   * etc.
   *
   * <p>Escaping any other character (including the escape character itself), just results in that
   * character in the pattern. e.g., "\a" means "a" and "\\" means "\"
   *
   * <p>If invoking the StringMatcher with string literals in Java, don't forget escape characters
   * are represented by "\\".
   *
   ...
   *     ending positions (exclusive) of the first occurrence of the pattern in the specified range
   *     of the text; return null if not found or subtext is empty (start==end). A pair of zeros is
   *     returned if pattern is empty string Note that for pattern like "*abc*" with leading and
   *     trailing stars, position of "abc" is returned. For a pattern like"*??*" in text "abcdf",
   *     (1,3) is returned
   */
