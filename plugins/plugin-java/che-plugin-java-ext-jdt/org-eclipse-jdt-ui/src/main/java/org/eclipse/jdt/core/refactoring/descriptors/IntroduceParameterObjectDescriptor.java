	...
	protected void populateArgumentMap() {
		...
		for (int i= 0; i < parameters.length; i++) {
			...
			JavaRefactoringDescriptorUtil
					.setInt(fArguments, JavaRefactoringDescriptorUtil.getAttributeName(PARAMETER_IDX, i), param.getIndex());
			JavaRefactoringDescriptorUtil.setBoolean(fArguments, JavaRefactoringDescriptorUtil.getAttributeName(PARAMETER_CREATE_FIELD, i),
													 param.isCreateField());
			if (param.isCreateField())
				JavaRefactoringDescriptorUtil.setString(fArguments, JavaRefactoringDescriptorUtil.getAttributeName(PARAMETER_FIELD_NAME,
																												   i),
														param.getFieldName());
		}
	...
	private void initializeParameter(Map map) throws IllegalArgumentException {
		...
		for (int i= 0; i < idx.length; i++) {
			...
			if (index == PARAMETER_OBJECT_IDX) {
				...
				if (createField[i])
					parameter.setFieldName(JavaRefactoringDescriptorUtil
												   .getString(map, JavaRefactoringDescriptorUtil.getAttributeName(PARAMETER_FIELD_NAME, i)));
			}
		}
		...
		if (fParameters != null) {
			...
			for (int i= 0; i < fParameters.length; i++) {
				...
				if (parameter.isCreateField()) {
					...
					if (fieldName == null)
						result.addError("The parameter " + parameter.getIndex() + " is marked for field creation but does not have a field" +
