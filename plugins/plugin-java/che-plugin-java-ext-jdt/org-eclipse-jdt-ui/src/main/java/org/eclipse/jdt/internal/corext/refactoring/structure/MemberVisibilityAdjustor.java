...
public final class MemberVisibilityAdjustor {
	...
	public static final GroupCategorySet SET_VISIBILITY_ADJUSTMENTS =
			new GroupCategorySet(new GroupCategory("org.eclipse.jdt.internal.corext.visibilityAdjustments", //$NON-NLS-1$
	...
	private Map<IMember, IncomingMemberVisibilityAdjustment> fAdjustments =
			new LinkedHashMap<IMember, IncomingMemberVisibilityAdjustment>(); // LinkedHashMap to preserve order of generated warnings
	...
	public MemberVisibilityAdjustor(final IJavaElement referencing, final IMember referenced) {
		...
		fScope = RefactoringScopeFactory.createReferencedScope(new IJavaElement[]{referenced},
	 ...
	 */
	private void adjustIncomingVisibility(final IJavaElement element, IMember referencedMovedElement, final IProgressMonitor monitor)
		...
		if (declaring != null && declaring.isInterface() ||
			referencedMovedElement instanceof IField && Flags.isEnum(referencedMovedElement.getFlags()))
			return;
		if (hasLowerVisibility(flags, threshold == null ? Modifier.NONE : threshold.toFlagValue()) &&
			...
			fAdjustments.put(referencedMovedElement, new IncomingMemberVisibilityAdjustment(referencedMovedElement, threshold,
																							RefactoringStatus
																									.createStatus(fVisibilitySeverity,
																												  Messages.format(
																														  getMessage(
	...
	private SearchResultGroup[] findReferences(final IMember member, final IProgressMonitor monitor) throws JavaModelException {
		final RefactoringSearchEngine2 engine = new RefactoringSearchEngine2(
				SearchPattern.createPattern(member, IJavaSearchConstants.REFERENCES, SearchUtils.GENERICS_AGNOSTIC_MATCH_RULE));
	 ...
	 * @throws JavaModelException if an error occurs
	 */
	private void adjustOutgoingVisibility(final IMember member, final ModifierKeyword threshold, final String template)
			throws JavaModelException {
		...
		if (declaring != null && (JavaModelUtil.isInterfaceOrAnnotation(declaring)
			...
			adjust = false;
		if (adjust && hasLowerVisibility(member.getFlags(), keywordToVisibility(threshold)) && needsVisibilityAdjustment(member,
																														 threshold))
			fAdjustments.put(member, new OutgoingMemberVisibilityAdjustment(member, threshold, RefactoringStatus
					.createStatus(fVisibilitySeverity, Messages.format(template, new String[]{JavaElementLabels.getTextLabel(member,
	...
	private void adjustOutgoingVisibilityChain(final IMember member, final IProgressMonitor monitor) throws JavaModelException {
		...
		if (!Modifier.isPublic(member.getFlags())) {
			...
			if (member instanceof IMethod) {
				adjustOutgoingVisibility(member, threshold,
	...
	public final void adjustVisibility(final IProgressMonitor monitor) throws JavaModelException {
		try {
			...
			monitor.setTaskName(RefactoringCoreMessages.MemberVisibilityAdjustor_checking);
			final RefactoringSearchEngine2 engine = new RefactoringSearchEngine2(
					SearchPattern.createPattern(fReferenced, IJavaSearchConstants.REFERENCES, SearchUtils.GENERICS_AGNOSTIC_MATCH_RULE));
	 ...
	 * @throws JavaModelException if the java elements could not be accessed
	 */
	private ModifierKeyword thresholdTypeToMethod(final IType referencing, final IMethod referenced, final IProgressMonitor monitor) throws
																																	 JavaModelException {
		final ICompilationUnit referencedUnit= referenced.getCompilationUnit();
		ModifierKeyword keyword= ModifierKeyword.PUBLIC_KEYWORD;
		if (referenced.getDeclaringType().equals(referencing))
	 ...
	 * @throws JavaModelException if the java elements could not be accessed
	 */
	private ModifierKeyword thresholdTypeToType(final IType referencing, final IType referenced, final IProgressMonitor monitor) throws
																																 JavaModelException {
		ModifierKeyword keyword= ModifierKeyword.PUBLIC_KEYWORD;
		final ICompilationUnit referencedUnit= referenced.getCompilationUnit();
