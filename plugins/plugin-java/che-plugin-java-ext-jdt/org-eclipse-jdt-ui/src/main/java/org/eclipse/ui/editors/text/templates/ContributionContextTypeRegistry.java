...
//		members.addResolver(new ExceptionVariableNameResolver());
	...
	private static TemplateContextType createContextType(IConfigurationElement element) throws CoreException {
		...
		try {
			...
			throw new CoreException(new Status(IStatus.ERROR, "org.eclipse.ui.editors", IStatus.OK, "extension does not implement " + TemplateContextType.class.getName(), e)); //$NON-NLS-1$
		}
	}
	...
	private static TemplateVariableResolver createResolver(IConfigurationElement element) throws CoreException {
		try {
			...
			throw new CoreException(new Status(IStatus.ERROR, "org.eclipse.ui.editors", IStatus.OK, "extension does not implement " + TemplateVariableResolver.class.getName(), e)); //$NON-NLS-1$
		}
