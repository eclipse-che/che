...
import {OpenshiftParser, IPodList, IPodItem, IPodItemContainer} from './openshift-parser';
...
export class OpenshiftEnvironmentManager extends EnvironmentManager {
   ...
   * @returns {Array<IPodItem>} recipe object
   */
  parseRecipe(content: string): IPodList | IPodItem {
    return this.parser.parse(content);
  }

  /**
   * Dumps recipe object.
   * @param recipe {Array<IPodItem>} recipe object
   * @returns {string} recipe content
   */

  stringifyRecipe(recipe: IPodList | IPodItem): string {
  ...
  getMachines(environment: che.IWorkspaceEnvironment, runtime?: any): Array<IEnvironmentManagerMachine> {
    ...
    if (environment && environment.recipe && environment.recipe.content) {
      let recipe: IPodList;
      try {
        recipe = <IPodList>this.parseRecipe(environment.recipe.content);
  ...
  getEnvironment(environment: che.IWorkspaceEnvironment, machines: IEnvironmentManagerMachine[]): che.IWorkspaceEnvironment {
    ...
    if (environment && environment.recipe && environment.recipe.content) {
      let recipe: IPodList;
      try {
        recipe = <IPodList>this.parseRecipe(newEnvironment.recipe.content);
      } catch (e) {
        this.$log.error('EnvironmentManager: cannot parse recipe.');
        return environment;
      }
  ...
  deleteMachine(environment: che.IWorkspaceEnvironment, name: string): che.IWorkspaceEnvironment {
    ...
    try {
      const envRecipe = <IPodList>this.parseRecipe(environment.recipe.content);
