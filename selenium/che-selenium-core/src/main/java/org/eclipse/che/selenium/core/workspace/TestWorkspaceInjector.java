...
public class TestWorkspaceInjector<T> implements MembersInjector<T> {
    ...
    public TestWorkspaceInjector(Field field,
                                 InjectTestWorkspace injectTestWorkspace,
    ...
    public void injectMembers(T instance) {
        ...
        int workspaceDefaultMemoryGb = injector.getInstance(Key.get(int.class,
                                                                    Names.named("workspace.default_memory_gb")));

        try {
            TestUser testUser = isNullOrEmpty(injectTestWorkspace.user())
                                ? injector.getInstance(DefaultTestUser.class)
                                : findInjectedUser(instance, injectTestWorkspace.user());


            int memoryGb = injectTestWorkspace.memoryGb() <= 0
                           ? workspaceDefaultMemoryGb
                           : injectTestWorkspace.memoryGb();

            TestWorkspace testWorkspace = testWorkspaceProvider.createWorkspace(testUser, memoryGb, injectTestWorkspace.template());
            testWorkspace.await();

            field.setAccessible(true);
            field.set(instance, testWorkspace);
        } catch (Exception e) {
            throw new RuntimeException("Failed to instantiate workspace in " + instance.getClass().getName(), e);
        }
    }
    ...
    private TestUser findInjectedUser(T instance, String userEmail) {
        ...
        throw new IllegalArgumentException(format("User %s not found or isn't instantiated in %s",
                                                  userEmail,
