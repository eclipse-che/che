...
public class TestGitHubServiceClient {
    ...
    public void deletePublicKeys(final String username,
                                 final String password,
                                 final String keyTitle) throws Exception {
        List<GitHubKey> keys = getPublicKeys(username, password, keyTitle);
        for (GitHubKey key : keys) {
            requestFactory.fromUrl(key.getUrl())
    ...
    public void createPublicKey(final String username,
                                final String password,
                                final GitHubKey key) throws Exception {
        requestFactory.fromUrl("https://api.github.com/user/keys")
                      .setAuthorizationHeader(createBasicAuthHeader(username, password))
                      .usePostMethod()
                      .setBody(key)
                      .request();
    ...
    public void uploadPublicKey(final String username,
                                final String password,
    ...
    public List<GitHubKey> getPublicKeys(final String username,
                                         final String password,
                                         final String title) throws Exception {
        List<GitHubKey> keys = requestFactory.fromUrl("https://api.github.com/user/keys")
                                             .setAuthorizationHeader(createBasicAuthHeader(username, password))
                                             .useGetMethod()
                                             .request()
        ...
        return keys.stream()
                   .filter(key -> title.equals(key.getTitle()))
                   .collect(toList());
    }


    public void hardResetHeadToCommit(final String repository,
                                      final String commitSha,
                                      final String username,
                                      final String password) throws Exception {
        ImmutableMap<String, Object> m = ImmutableMap.of("sha", commitSha, "force", true);

        String url = "https://api.github.com/repos/" + username + "/" + repository + "/git/refs/heads/master";
    ...
    public List<String> getNumbersOfOpenedPullRequests(final String repository,
                                                       final String username,
                                                       final String password) throws Exception {
        ...
        HttpJsonResponse response = requestFactory.fromUrl(url)
                                                  ...
                                                  .setAuthorizationHeader(createBasicAuthHeader(username, password))
                                                  .request();
        List<Map<String, String>> prs = response.as(List.class, new TypeToken<List<Map<String, String>>>() {}.getType());
        return prs.stream().filter(g -> g.get("state").equals("open"))
                           .map(g -> g.get("number")).collect(toList());
    }

    public void closePullRequest(final String repo,
                                 final String number,
                                 final String username,
                                 final String password) throws Exception {
    ...
    public void deleteBranch(final String repository,
                             final String branchName,
                             final String username,
                             final String password) throws Exception {
        String url = "https://api.github.com/repos/" + username + "/" + repository + "/git/refs/heads/" + branchName;
        requestFactory.fromUrl(url)
                      .useDeleteMethod()
                      .setAuthorizationHeader(createBasicAuthHeader(username, password))
                      .request();
    ...
    public void deleteRepo(final String repository,
                           final String username,
                           final String password) throws Exception {
    ...
    public List<String> getAllGrants(final String username,
                                     final String password) throws Exception {
        ...
        HttpJsonResponse response = requestFactory.fromUrl(url)
                                                  ...
                                                  .request();
        @SuppressWarnings("unchecked")
        List<Map<String, String>> grants = response.as(List.class, new TypeToken<List<Map<String, String>>>() {}.getType());
