...
/**
 * Represent in memory based storage of test configuration.
 * Values of configuration load in following sequence:
 ...
 * 4. From environment variables (will be given prefix "env.").
 * "_" is replaced by "." in names.
 * Variables which start from "che." or "codenvy." aren't been given additional prefix "sys." or "env.".
 */
@Singleton
public class SeleniumTestConfiguration extends InMemoryTestConfiguration {
    ...
    static class SystemPropertiesConfiguration extends InMemoryTestConfiguration {
        ...
        SystemPropertiesConfiguration() {
            super();
            addAll(System.getProperties().entrySet().stream()
    ...
    static class EnvironmentVariableToSystemPropertyFormatNameConverter implements
                                                                        Function<Map.Entry<String, String>, Map.Entry<String, String>> {
        @Override
