 ...
 */
@Singleton
public class SeleniumWebDriver implements Closeable, WebDriver, JavascriptExecutor, TakesScreenshot, HasInputDevices {
    ...
    @Inject
    public SeleniumWebDriver(@Named("sys.browser") TestBrowser browser,
        ...
        try {
            URL webDriverUrl = new URL(format("http://localhost:%s%s", webDriverPort, gridMode ? "/wd/hub" : ""));
    ...
    private String getGoogleChromeTip() {
        try {
            URL webDriverNotes = new URL(String.format("http://chromedriver.storage.googleapis.com/%s/notes.txt", webDriverVersion));
            String supportedVersions = readSupportedVersionInfoForGoogleDriver(webDriverNotes);
            if (supportedVersions != null) {
                return format("%n(Tip: there is Chrome Driver v.%s used, and it requires local Google Chrome of v.%s)",
                              webDriverVersion, supportedVersions);
            }
        } catch (java.io.IOException e) {
            LOG.warn("It's impossible to read info about versions of browser which Chrome Driver supports.", e);
        }

        return format("%n(Tip: check manually if local Google Chrome browser supported by Chrome Driver v.%s at official site)",
                      webDriverVersion);
    }
     ...
     *
     * @param webDriverOfficialNotes
     *         address of official page with Google driver info
     * @return string with supported version range (for example, "36-40"), or null if version info doesn't found inside the official notes.
     * @throws IOException
     */
    @Nullable
    private String readSupportedVersionInfoForGoogleDriver(URL webDriverOfficialNotes) throws IOException {
    ...
    public void waitOpenedSomeWin() {
        new WebDriverWait(this, 30).until((ExpectedCondition<Boolean>)input -> {
    ...
    /**
     * calculate name of workspace from browser url
     * cut symbols from end of slash symbol ("/") to end
     *
     * @return
     */
    ...
    /**
     * switch to the next browser window (this means that if  opened 2 windows, and we are in the window 1, we will be switched into the
     * window 2 )
     *
    ...
    public void switchFromDashboardIframeToIde() {
        new WebDriverWait(this, EXPECTED_MESS_IN_CONSOLE_SEC).until
                (ExpectedConditions.frameToBeAvailableAndSwitchToIt(By.id("ide-application-iframe")));
    }

    public void switchFromDashboardIframeToIde(int timeout) {
        new WebDriverWait(this, timeout).until(ExpectedConditions.frameToBeAvailableAndSwitchToIt(By.id("ide-application-iframe")));
    }
