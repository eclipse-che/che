...
public class PullRequestPluginTest {
    ...
    @Inject
    private AskForValueDialog                valueDialog;
    @Inject
    ...
    public void tearDown() throws Exception {
        ...
        gitHubClientService.closePullRequest(NAME_REPO, Collections.max(listPullRequest), gitHubUsername, gitHubPassword);
    ...
    public void switchingBetweenProjects() {
        ...
        String firstProjectUrl = "https://github.com/" + gitHubUsername + "/pull-request-plugin-test.git";
    ...
    public void updatePullRequest() {
        ...
        pullRequestPanel.clickOkCommitBtn();
        String expectedText = "Branch '" + gitHubUsername + ":" + NEW_BRANCH + "' is already used. Would you like to overwrite it?";
    ...
    private void checkGitHubUserPage() {
        webDriverWait.until(visibilityOfElementLocated(By.xpath("//h1//a[text()='" + gitHubUsername + "']")));
        webDriverWait.until(visibilityOfElementLocated(By.xpath("//h1//a[text()='" + FIRST_PROJECT_NAME + "']")));
        webDriverWait.until(visibilityOfElementLocated(By.xpath("//h1//span[contains(text(), '" + TITLE + "')]")));
        webDriverWait.until(visibilityOfElementLocated(By.xpath("//span[text()='" + NEW_BRANCH + "']")));
        webDriverWait.until(visibilityOfElementLocated(By.xpath("//p[text()='" + COMMENT + "']")));
    }

    private void consumeFactoryOnGitHub() {
        webDriverWait.until(elementToBeClickable(By.xpath("//a[contains(@href, 'id=factory')]"))).click();
    }
