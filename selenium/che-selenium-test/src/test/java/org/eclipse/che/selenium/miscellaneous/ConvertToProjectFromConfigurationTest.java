...
public class ConvertToProjectFromConfigurationTest {
    ...
    private static final String pathToJsFile    = PROJECT_NAME + "/src/main/webapp/WEB-INF/" + JS_FOLDER_NAME;

    private static final String TEXT_FILE_JS  = "var express = require('express');\n" +
                                                "var app = express();\n" +
                                                "\n" +
    ...
    public void checkConvertToProjectFromConfiguration() throws Exception {
        ...
        createNewFile("file.php", pathToPhpFile, TestProjectExplorerContextMenuConstants.SubMenuNew.FILE);
        ...
        menu.runCommand(TestMenuCommandsConstants.Project.PROJECT, TestMenuCommandsConstants.Project.CONFIGURATION);
        ...
        createNewFile("fileJS", pathToJsFile, TestProjectExplorerContextMenuConstants.SubMenuNew.JAVASCRIPT_FILE);
    ...
    public void convertToProject(String folderName, String parentDirectory) {
        ...
        projectExplorer.clickOnItemInContextMenu(TestProjectExplorerContextMenuConstants.CONVERT_TO_PROJECT);
        wizard.waitOpenProjectConfigForm();
    ...
    private String getContentFromFile(String path) throws Exception {
        List<String> textFromFile = Files.readAllLines(Paths.get(getClass().getResource(path).toURI()), Charset.forName("UTF-8"));
        return Joiner.on("\n").join(textFromFile);
    }
}
