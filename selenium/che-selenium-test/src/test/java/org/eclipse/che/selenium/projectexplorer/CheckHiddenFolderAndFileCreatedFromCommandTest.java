...
public class CheckHiddenFolderAndFileCreatedFromCommandTest {
    private static final String PROJECT_NAME               = CheckHiddenFolderAndFileCreatedFromCommandTest.class.getSimpleName();
    ...
    /**
     * The 'checkHiddenFolderAndFile' test:
     * 1. Open project and set 'SHOW_HIDE_HIDDEN_FILES' in  the 'Project' menu to show hidden files;
     * 2. Start "createHiddenFolder" command which create a ".res" folder and verify if it exists;
     * 3. Start "createHiddenFile" command which create a ".text" folder and verify if it exists and have text(result of 'pwd').
     */
    @Test
    public void checkHiddenFolderAndFile() throws Exception {
        ...
        menu.runCommand(TestMenuCommandsConstants.Project.PROJECT,
                        TestMenuCommandsConstants.Project.SHOW_HIDE_HIDDEN_FILES); // all hidden files and folders are visible
        projectExplorer.invokeCommandWithContextMenu(ProjectExplorer.CommandsGoal.COMMON,
                                                     PROJECT_NAME,
                                                     COMMAND_CREATE_FOLDER_NAME);
        loader.waitOnClosed();
        ...
        projectExplorer.waitDisappearItemByPath(PATH_TO_FILE); // all hidden files and folders are hidden
