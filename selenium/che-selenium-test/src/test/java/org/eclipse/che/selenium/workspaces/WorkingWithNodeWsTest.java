...
public class WorkingWithNodeWsTest {
    ...
    public void checkNodeJsWsAndRunApp() {
        ...
        projectExplorer
                .invokeCommandWithContextMenu(ProjectExplorer.CommandsGoal.BUILD, NODE_JS_PROJECT_NAME,
                                              "web-nodejs-simple:install dependencies");
        consoles.waitExpectedTextIntoConsole("bower_components/angular", 400);
        projectExplorer
                .invokeCommandWithContextMenu(ProjectExplorer.CommandsGoal.RUN, NODE_JS_PROJECT_NAME, "web-nodejs-simple:run");
        loader.waitOnClosed();
    ...
    public void checkAngularYeomanAppl() {
        new WebDriverWait(ide.driver(), LOAD_PAGE_TIMEOUT_SEC).until(
                ExpectedConditions.visibilityOfElementLocated(By.xpath("//h1[text()=\"'Allo, 'Allo!\"]")));
        new WebDriverWait(ide.driver(), LOAD_PAGE_TIMEOUT_SEC).until(
                ExpectedConditions.visibilityOfElementLocated(By.xpath("//img[@src='images/yeoman.png']")));
        new WebDriverWait(ide.driver(), LOAD_PAGE_TIMEOUT_SEC).until(
                ExpectedConditions.visibilityOfElementLocated(By.linkText("Splendid!"))).click();
    }
}
