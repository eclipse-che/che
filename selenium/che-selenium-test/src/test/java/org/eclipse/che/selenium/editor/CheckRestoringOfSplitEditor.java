...
public class CheckRestoringOfSplitEditor {
    ...
    public void prepare() throws Exception {
        URL resources = CheckRestoringOfSplitEditor.class.getResource("split-editor-restore-exp-text.txt");
    ...
    public void checkRestoringStateSplittedEditor() throws IOException {
        ...
        checkSplitdEditorAfterRefreshing(1, nameJavaClass.split("\\.")[0], expectedText.get(0), cursorPositionForJavaFile);
        checkSplitdEditorAfterRefreshing(2, nameReadmeFile, expectedText.get(1).trim(), cursorPositionForReadMeFile);
        checkSplitdEditorAfterRefreshing(3, nameOfTabPomFile, expectedText.get(2).trim(), cursorPositionForPomFile);
    }

    private void checkSplitdEditorAfterRefreshing(int numOfEditor,
                                                  String nameOfEditorTab,
                                                  String expectedTextAfterRefresh,
                                                  Pair<Integer, Integer> pair) throws IOException {

        editor.waitActiveEditor();
        editor.selectTabByName(nameOfEditorTab);
        editor.waitSpecifiedValueForLineAndChar(pair.first, pair.second);
        editor.waitTextInDefinedSplitEditor(numOfEditor, REDRAW_UI_ELEMENTS_TIMEOUT_SEC, expectedTextAfterRefresh);
    }
    ...
    private void setPositionsForSplittedEditor() {
        ...
        editor.setCursorToDefinedLineAndChar(cursorPositionForJavaFile.first, cursorPositionForJavaFile.second);
        editor.selectTabByName(nameReadmeFile);
        editor.setCursorToDefinedLineAndChar(cursorPositionForReadMeFile.first, cursorPositionForReadMeFile.second);
        editor.selectTabByName(nameOfTabPomFile);
        editor.setCursorToDefinedLineAndChar(cursorPositionForPomFile.first, cursorPositionForPomFile.second);
    }
}
