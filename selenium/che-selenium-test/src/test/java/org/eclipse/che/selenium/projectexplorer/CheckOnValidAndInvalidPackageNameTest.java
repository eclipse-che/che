...
/**
 * Test tries to create packages with valid and invalid names inside
...
public class CheckOnValidAndInvalidPackageNameTest {
    ...
    public void prepare() throws Exception {
        ...
        testProjectServiceClient.importProject(testWorkspace.getId(), defaultTestUser.getAuthToken(), Paths.get(resource.toURI()), PROJECT_NAME,
    ...
    public void createValidPackageNameInJavaFolderTest(String packageName) {
        createPackageByPath(PROJECT_NAME + PATH_TO_JAVA_FOLDER,
                            packageName);
    ...
    public void createValidPackageNameInRootPackageTest(String packageName) {
        createPackageByPath(PROJECT_NAME + PATH_TO_JAVA_FOLDER + ROOT_PACKAGE,
                            packageName + PREFIX);
    ...
    public void createInvalidPackageNameInJavaFolderTest(String packageName) {
        ...
        if (!askForValueDialog.waitErrorMessage()) {
            ...
            Assert.fail("Expected error message not shown , user able to create package with invalid name");
        }
        askForValueDialog.clickCancelBtn();
        askForValueDialog.waitFormToClose();
    }
    ...
    public void createInvalidPackageNameInRootPackageTest(String packageName) {
        projectExplorer.openContextMenuByPathSelectedItem(PROJECT_NAME + PATH_TO_JAVA_FOLDER + ROOT_PACKAGE);
        projectExplorer.clickOnItemInContextMenu(TestProjectExplorerContextMenuConstants.NEW);
        projectExplorer.clickOnNewContextMenuItem(TestProjectExplorerContextMenuConstants.SubMenuNew.JAVA_PACKAGE);
        askForValueDialog.waitFormToOpen();
        askForValueDialog.typeAndWaitText(packageName);
    ...
    private void createPackageByPath(String packagePath,
                                     String packageName) {
