...
public class WorkingWithTerminalTest {
    ...
    private static final String[] CHECK_MC_OPENING =
            {"Left", "File", "Command", "Options", "Right", "Name", "/bin", "/boot", "/dev", "/etc", "/home", "/lib", "/lib64",
             "/bin", "1Help", "2Menu", "3View", "4Edit", "5Copy", "6RenMov", "7Mkdir", "8Delete", "9PullDn", "10Quit"};

    private static final String MESS_IN_CONSOLE = "Installing /projects/" + PROJECT_NAME + "/target/qa-spring-sample-1.0-SNAPSHOT.war";
    private static final String WAR_NAME        = "qa-spring-sample-1.0-SNAPSHOT.war";

    private static final String BASH_SCRIPT = "for i in `seq 1 10`; do sleep 5; echo \"test=$i\"; done";

    private static final String   MC_HELP_DIALOG      = "This is the main help screen for GNU Midnight Commander.";
    ...
    public void shouldEditFileIntoMCEdit() {
        ...
        terminal.waitExpectedTextIntoTerminal("README.md          " +
                                              "[----]  0 L:[  1+ 0   1/  1] *(0   /  21b) 0035 0x023");
        terminal.typeIntoTerminal("<!-some comment->");
        terminal.typeIntoTerminal("" + Keys.HOME + Keys.ARROW_RIGHT + Keys.ARROW_RIGHT + Keys.ARROW_RIGHT + Keys.DELETE);
