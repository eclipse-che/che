...
/**
 * Test tries to create classes with valid and invalid names inside
 * source root folder and inside another package.
 *
...
public class CheckOnValidAndInvalidClassNameTest {
    ...
    public void shouldCreateClassWithValidNameInJavaFolder(String className) {
        createJavaClassByPath(PROJECT_NAME + PATH_TO_JAVA_FOLDER,
                              className);

        projectExplorer.waitItem(PROJECT_NAME + PATH_TO_JAVA_FOLDER + "/" + className + TYPE,
    ...
    public void shouldCreateClassWithValidNameInRootPackage(String className) {
        createJavaClassByPath(PROJECT_NAME + PATH_TO_JAVA_FOLDER + ROOT_PACKAGE,
                              className);

        projectExplorer.waitItem(PROJECT_NAME + PATH_TO_JAVA_FOLDER + ROOT_PACKAGE + "/" + className + TYPE,
                                 REDRAW_UI_ELEMENTS_TIMEOUT_SEC);
    }
    ...
    public void shouldNotCreateClassWithInvalidNameInJavaFolder(String className) {
        tryToCreateJavaClassWithNotValidNameByPath(PROJECT_NAME + PATH_TO_JAVA_FOLDER,
                                                   className);
    }
    ...
    public void shouldNotCreateClassWithInvalidNameInRootPackage(String className) {
        tryToCreateJavaClassWithNotValidNameByPath(PROJECT_NAME + PATH_TO_JAVA_FOLDER + ROOT_PACKAGE,
                                                   className);
    }

    private void createJavaClassByPath(String classPath,
                                       String className) {
        ...
        projectExplorer.clickOnNewContextMenuItem(TestProjectExplorerContextMenuConstants.SubMenuNew.JAVA_CLASS);
    ...
    private void tryToCreateJavaClassWithNotValidNameByPath(String elementPath,
                                                            String className) {
