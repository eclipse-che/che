...
public class CheckFactoryWithSincePolicyTest {
    ...
    private static final String ALERT_EXPIRE_MESSAGE = "Error: This Factory is not yet valid due to time restrictions applied" +
                                                       " by its owner. Please, contact owner for more information.";
    private final static long   INIT_TIME            = System.currentTimeMillis();
    private final static int    ADDITIONAL_TIME      = 60000;
    ...
    public void checkFactoryAcceptingWithSincePolicy() throws Exception {
        ...
        new WebDriverWait(ide.driver(), PREPARING_WS_TIMEOUT_SEC).until(ExpectedConditions.alertIsPresent());
        assertTrue(ide.driver().switchTo().alert().getText().contains(ALERT_EXPIRE_MESSAGE),
