...
public class CheckMacrosFeatureTest {
    ...
    private static final String COMMAND      = "echo ${editor.current.file.name} ${editor.current.file.path}" +
    ...
    private              String expectedText = "README.md /projects/CheckMacrosFeatureTest/README.md" +
    ...
    public void checkMacrosFeature() throws Exception {
        ...
        projectExplorer.invokeCommandWithContextMenu(ProjectExplorer.CommandsGoal.COMMON, PROJECT_NAME, COMMAND_NAME);
    ...
    private void createCommand(String commandName, String command) {
        ...
        loader.waitOnClosed();
        commandsExplorer.chooseCommandTypeInContextMenu(TestIntelligentCommandsConstants.CommandsTypes.CUSTOM_TYPE);
        loader.waitOnClosed();
        commandsExplorer.waitCommandInExplorerByName(TestIntelligentCommandsConstants.CommandsDefaultNames.CUSTOM_NAME);
        commandsEditor.waitActiveEditor();
        commandsEditor.waitTabFileWithSavedStatus(TestIntelligentCommandsConstants.CommandsDefaultNames.CUSTOM_NAME);
        commandsEditor.clickOnCancelCommandEditorButton();
        commandsEditor.waitTabIsNotPresent(TestIntelligentCommandsConstants.CommandsDefaultNames.CUSTOM_NAME);

        commandsExplorer.waitCommandExplorerIsOpened();
        commandsExplorer.selectCommandByName(TestIntelligentCommandsConstants.CommandsDefaultNames.CUSTOM_NAME);
        editor.waitActiveEditor();
        commandsEditor.typeTextIntoNameCommandField(commandName);
        commandsEditor.waitTextIntoNameCommandField(commandName);
        commandsEditor.waitTabCommandWithUnsavedStatus(TestIntelligentCommandsConstants.CommandsDefaultNames.CUSTOM_NAME);
