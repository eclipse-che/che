...
public class Swagger {
    ...
    private void expandWorkSpaceItem() {
        new WebDriverWait(seleniumWebDriver, LOAD_PAGE_TIMEOUT_SEC).until(
                ExpectedConditions.elementToBeClickable(workSpaceLink)).click();
    }
    ...
    private void collapseWorkSpaceItem() {
        new WebDriverWait(seleniumWebDriver, LOAD_PAGE_TIMEOUT_SEC).until(
                ExpectedConditions.elementToBeClickable(workSpaceLink)).click();
        new WebDriverWait(seleniumWebDriver, LOAD_PAGE_TIMEOUT_SEC).until(
                ExpectedConditions.invisibilityOfElementLocated(By.id("workspace_endpoint_list")));
    }
    ...
    private void clickElementByXpath(String xPath) {
        new WebDriverWait(seleniumWebDriver, LOAD_PAGE_TIMEOUT_SEC).until(
                ExpectedConditions.elementToBeClickable(By.xpath(xPath))).click();
    }
    ...
    public String getWsNameFromWorkspacePage()  {
        ...
        String json = new WebDriverWait(seleniumWebDriver, LOAD_PAGE_TIMEOUT_SEC).until(
                ExpectedConditions.visibilityOfElementLocated(By.xpath(Locators.RESPONSE_BODY))).getText();
        List<WorkspaceDto> workspaces = DtoFactory.getInstance().createListDtoFromJson(json, WorkspaceDto.class);
     ...
     *         number of port
     * @param machineToken
     *         authorization token for machine
     * @param workspaceName
     *         name of workspace
     * @return String
     * @throws JsonParseException
     */
    public String createURLByExposedPort(String port, String machineToken, String workspaceName) throws JsonParseException {
        String json;
        String url;
        String serverAddress = "";
        expandWorkSpaceItem();
        clickTryItOutByXpath(Locators.TRY_IT_OUT_RUNTIME);
        json = new WebDriverWait(seleniumWebDriver, REDRAW_UI_ELEMENTS_TIMEOUT_SEC).until(
                ExpectedConditions.visibilityOfElementLocated(By.xpath(Locators.RESPONSE_BODY_RUNTIME))).getText();
        ...
        if (foundValue != null) {
            serverAddress = foundValue.getElement("runtime").getElement("devMachine").getElement("runtime")
                                      .getElement("servers").getElement(port).getElement("url").toString().replaceAll("\"", "");
        }
    ...
    private void clickOnProjectLink() {
        new WebDriverWait(seleniumWebDriver, LOAD_PAGE_TIMEOUT_SEC).until(
                ExpectedConditions.elementToBeClickable(By.xpath(Locators.PROJECT))).click();
    }
    ...
    private void enterURLAndClickExplore(String URL) {
        new WebDriverWait(seleniumWebDriver, REDRAW_UI_ELEMENTS_TIMEOUT_SEC).until(
                ExpectedConditions.visibilityOfElementLocated(By.xpath(Locators.BASE_URL_INPUT))).clear();
        new WebDriverWait(seleniumWebDriver, REDRAW_UI_ELEMENTS_TIMEOUT_SEC).until(
                ExpectedConditions.visibilityOfElementLocated(By.xpath(Locators.BASE_URL_INPUT))).sendKeys(URL);
        new WebDriverWait(seleniumWebDriver, REDRAW_UI_ELEMENTS_TIMEOUT_SEC).until(
                ExpectedConditions.visibilityOfElementLocated(By.xpath(Locators.EXPLORE))).click();
    }
    ...
    public List<String> getNamesOfProjects(String URL) throws JsonParseException {
        ...
        json = new WebDriverWait(seleniumWebDriver, REDRAW_UI_ELEMENTS_TIMEOUT_SEC).until(
                ExpectedConditions.visibilityOfElementLocated(By.xpath(Locators.GET_WS_ID_RESPONSE_BODY))).getText();
        Iterator<JsonValue> jsonHelperIterator = JsonHelper.parseJson(json).getElements();

        while (jsonHelperIterator.hasNext()) {
            projectsList.add(jsonHelperIterator.next().getElement("name").toString().replaceAll("\"", ""));
        }
