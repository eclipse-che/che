...
/**
 * The ordinary object which describes dialog window with references to selected item, and contains business
 * logic which allows to find necessary {@link WebElement}, get values from them or compare values from web element to expected
 * values.
 *
...
public class ShowReference {
    ...
    public void checkFqnFieldValue(String expectedVal) {
        new WebDriverWait(seleniumWebDriver, REDRAW_UI_ELEMENTS_TIMEOUT_SEC).until(textToBePresentInElementValue(fqnTextBox, expectedVal));
    }
    ...
    public void checkPathFieldValue(String expectedVal) {
        new WebDriverWait(seleniumWebDriver, REDRAW_UI_ELEMENTS_TIMEOUT_SEC).until(textToBePresentInElementValue(pathTextBox, expectedVal));
    }
