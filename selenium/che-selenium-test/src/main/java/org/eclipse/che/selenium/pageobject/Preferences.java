...
public class Preferences {
    ...
    public void waitPreferencesForm() {
        new WebDriverWait(seleniumWebDriver, 10).until(ExpectedConditions.visibilityOf(preferencesForm));
    }
     ...
     *
     * @param nameMenu
     *         name of header (all names describe in public interface )
     */
    public void waitDropDownHeaderMenu(String nameMenu) {
        new WebDriverWait(seleniumWebDriver, 20).until(ExpectedConditions.visibilityOfAllElementsLocatedBy(
     ...
     *
     * @param menu
     *         (all menus describe in public interface )
     */
    ...
    public void selectDroppedMenuByName(String nameMenu) {
        ...
        seleniumWebDriver.findElement(By.xpath(format(Locators.MENU_IN_EXPANDED_DROPDOWN_XPATH_WITH_PARAM, nameMenu))).click();
    }
    ...
    public void clickOnGenerateKeyButton() {
        new WebDriverWait(seleniumWebDriver, 10).until(ExpectedConditions.visibilityOf(generateKeyBtn)).click();
    }

    public void clickOnGenerateAndUploadToGitHub() {
        new WebDriverWait(seleniumWebDriver, 10).until(ExpectedConditions.visibilityOf(generateAndUploadBtn)).click();
    }

    public boolean isSshKeyTableIsEmpty(String expText) {
        loader.waitOnClosed();
        new WebDriverWait(seleniumWebDriver, ELEMENT_TIMEOUT_SEC).until(
                ExpectedConditions.presenceOfElementLocated(By.xpath(Locators.SSH_KEYS_TABLE)));
    ...
    public boolean isSshKeyIsPresent(String host) {
        loader.waitOnClosed();
        new WebDriverWait(seleniumWebDriver, ELEMENT_TIMEOUT_SEC).until(
                ExpectedConditions.visibilityOfElementLocated(By.xpath(Locators.SSH_KEYS_TABLE)));
        return sshKeysTable.getText().contains(host);
    }

    public void waitSshKeyIsPresent(final String host) {
        new WebDriverWait(seleniumWebDriver, ELEMENT_TIMEOUT_SEC).until(
                (ExpectedCondition<Boolean>)driver -> sshKeysTable.getText().contains(host));
    }

    public void deleteSshKeyByHost(String host) {
        WebElement element = seleniumWebDriver.findElement(By.xpath("//div[text()='" + host + "']" + Locators.SSH_DELETE_BUTTON_FOR_HOST));
        try {
            new WebDriverWait(seleniumWebDriver, ELEMENT_TIMEOUT_SEC).until(ExpectedConditions.visibilityOf(element)).click();
    ...
    public List<String> getItemsFromErrorWarningsWidget() {
        List<String> itemList = Arrays.asList(new WebDriverWait(seleniumWebDriver, REDRAW_UI_ELEMENTS_TIMEOUT_SEC).until(
                ExpectedConditions.visibilityOf(errorsWarningsTab)).getText().split(
     ...
     *
     * @param valueOfDropDown
     *         value of drop down
     */
    public void setAllSettingsInErrorWaitingWidget(DropDownValueForErrorWaitingWidget valueOfDropDown) {
        ...
        for (String settingsText : getItemsFromErrorWarningsWidget()) {
            ...
            new WebDriverWait(seleniumWebDriver, REDRAW_UI_ELEMENTS_TIMEOUT_SEC).until(ExpectedConditions.visibilityOfElementLocated(
                    By.xpath(format(Locators.ERRORS_WARNINGS_RADIO_BUTTON_BLOCK, settingsText)))).click();
    ...
    public List<String> getAllSettingsFromEditorWidget(String[] headerSettings) {
        List<String> settingList =
                new ArrayList<>(Arrays.asList(new WebDriverWait(seleniumWebDriver, REDRAW_UI_ELEMENTS_TIMEOUT_SEC).until(
                        ExpectedConditions.visibilityOf(editorProperties)).getText().split("\n")));
    ...
    public void setAllCheckboxSettingsInEditorWidget(FlagForEditorWidget valueOfFlag, List<String> settingsList) {
        for (String settingsItem : settingsList) {
            if (new WebDriverWait(seleniumWebDriver, REDRAW_UI_ELEMENTS_TIMEOUT_SEC).until(ExpectedConditions.presenceOfElementLocated(
                    By.xpath(format(Locators.EDITOR_INPUT, settingsItem)))).getAttribute("type").equals("checkbox")) {
                WebElement checkbox =
                        new WebDriverWait(seleniumWebDriver, REDRAW_UI_ELEMENTS_TIMEOUT_SEC)
                                .until(ExpectedConditions.presenceOfElementLocated(
                                        By.xpath(format(Locators.EDITOR_INPUT, settingsItem))));
                WebElement spanCheckbox = new WebDriverWait(seleniumWebDriver, REDRAW_UI_ELEMENTS_TIMEOUT_SEC)
                        .until(ExpectedConditions.presenceOfElementLocated(
    ...
    public void uploadPrivateSshKey(String host, String filePath) {
        ...
        new WebDriverWait(seleniumWebDriver, REDRAW_UI_ELEMENTS_TIMEOUT_SEC)
                .until(ExpectedConditions.visibilityOfElementLocated(By.xpath(Locators.SSH_UPLOAD_KEY))).click();
        WebElement hostInput = new WebDriverWait(seleniumWebDriver, REDRAW_UI_ELEMENTS_TIMEOUT_SEC)
                .until(ExpectedConditions.visibilityOfElementLocated(By.xpath(UploadSSHKey.HOST_INPUT)));
        hostInput.clear();
        hostInput.sendKeys(host);
        new WebDriverWait(seleniumWebDriver, REDRAW_UI_ELEMENTS_TIMEOUT_SEC)
                .until(ExpectedConditions.visibilityOfElementLocated(By.xpath(UploadSSHKey.FILE_INPUT))).sendKeys(file.getAbsolutePath());
        WaitUtils.sleepQuietly(3);
        new WebDriverWait(seleniumWebDriver, REDRAW_UI_ELEMENTS_TIMEOUT_SEC)
                .until(ExpectedConditions.visibilityOfElementLocated(By.xpath(UploadSSHKey.UPLOAD_BUTTON))).click();
    }
     ...
     *
     * @param title
     *         the name of title for created Ssh key
     */
    public void typeTitleOfSshKey(String title) {
        new WebDriverWait(seleniumWebDriver, REDRAW_UI_ELEMENTS_TIMEOUT_SEC)
                .until(ExpectedConditions.visibilityOf(genrateSShKeyTitleInput)).sendKeys(
                title);
    }
    ...
    /**
     * wait opening the  Generate Ssh key eneter the title ckick on ok btn and wait closing the widget. Wait that Ssh key presents in the
     * list.
     *
     * @param titleOfKey
     */
    ...
    public void regenerateAndUploadSshKeyOnGithub(String githubUsername, String githubPassword) throws Exception {
