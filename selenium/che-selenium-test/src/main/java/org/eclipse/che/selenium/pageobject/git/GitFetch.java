...
public class GitFetch {
    ...
    @FindBy
            (id = Locators.FETCH_BTN_ID)
    WebElement fetchBtn;

    @FindBy
            (id = Locators.REMOTE_REPOSITORY_SELECT)
    WebElement remoteRepositorySelect;

    @FindBy
            (id = Locators.REMOTE_BRANCH_SELECT)
    WebElement remoteBranchSelect;

    @FindBy
            (id = Locators.LOCAL_BRANCH_SELECT)
    WebElement localBranchSelect;

    @FindBy
            (id = Locators.REMOVE_DELETED_REFS)
    WebElement removeDeletedRefs;

    @FindBy
            (id = Locators.FETCH_ALL_BRANCHES)
    ...
    public void waitFormToClose() {
        new WebDriverWait(seleniumWebDriver, 5).until(ExpectedConditions.invisibilityOfElementLocated(By.id(Locators.FETCH_FORM)));
    }

    public void clickFetchBtn() {
        new WebDriverWait(seleniumWebDriver, 5).until(ExpectedConditions.elementToBeClickable(fetchBtn));
        fetchBtn.click();
    }
    ...
    public void waitFetchBtnIsDisabled() {
        new WebDriverWait(seleniumWebDriver, 5).until(ExpectedConditions.not(ExpectedConditions.elementToBeClickable(fetchBtn)));
    }

    /**
     * click on remote branch
     * wait the list of names remote branches
     *
     * @param listNames
     *         is the list of names remote branches
     */
