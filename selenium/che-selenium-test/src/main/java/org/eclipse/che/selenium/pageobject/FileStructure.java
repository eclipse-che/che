...
public class FileStructure {
    ...
    @Inject
    public FileStructure(SeleniumWebDriver seleniumWebDriver, Loader loader, ActionsFactory actionsFactory) {
    ...
    private interface Locators {
        ...
        String FILE_STRUCTURE_ICON_NODE  = "(//div[text()='%s']/preceding::*[local-name()='svg'][2])[last()]";
        String FILE_STRUCTURE_ITEM       = "//div[@id='gwt-debug-file-structure-mainPanel']//div[text()='%s']";
    }

    @FindBy
            (id = Locators.FILE_STRUCTURE_CONTENT)
    ...
    public void waitFileStructureFormIsOpen(String fileName) {
        new WebDriverWait(seleniumWebDriver, REDRAW_UI_ELEMENTS_TIMEOUT_SEC).until
                (ExpectedConditions.visibilityOfElementLocated(By.xpath(String.format(Locators.FILE_STRUCTURE_FORM, fileName))));
    }
    ...
    public void clickFileStructureCloseIcon() {
        new WebDriverWait(seleniumWebDriver, REDRAW_UI_ELEMENTS_TIMEOUT_SEC).until
                (ExpectedConditions.visibilityOfElementLocated(By.xpath(Locators.FILE_STRUCTURE_CLOSE_ICON)))
                                                                       .click();
    }
    ...
    public void waitExpectedTextInFileStructure(String expText) {
        new WebDriverWait(seleniumWebDriver, REDRAW_UI_ELEMENTS_TIMEOUT_SEC).until(new ExpectedCondition<Boolean>() {
            @Override
    ...
    public void clickOnIconNodeInFileStructure(String nameNode) {
        ...
        new WebDriverWait(seleniumWebDriver, REDRAW_UI_ELEMENTS_TIMEOUT_SEC).until
                (ExpectedConditions.visibilityOfElementLocated(By.xpath(String.format(Locators.FILE_STRUCTURE_ICON_NODE, nameNode))))
                                                                       .click();
    }
    ...
    public void selectItemInFileStructureByDoubleClick(String item) {
        WebElement fileStructureItem = new WebDriverWait(seleniumWebDriver, REDRAW_UI_ELEMENTS_TIMEOUT_SEC).until
                (ExpectedConditions.visibilityOfElementLocated(By.xpath(String.format(Locators.FILE_STRUCTURE_ITEM, item))));
    ...
    public void selectItemInFileStructure(String item) {
        new WebDriverWait(seleniumWebDriver, REDRAW_UI_ELEMENTS_TIMEOUT_SEC).until
                (ExpectedConditions.visibilityOfElementLocated(By.xpath(String.format(Locators.FILE_STRUCTURE_ITEM, item)))).click();
    }
    ...
    public void moveDownToItemInFileStructure(String itemName) {
        List<WebElement> items = new WebDriverWait(seleniumWebDriver, REDRAW_UI_ELEMENTS_TIMEOUT_SEC)
                .until(ExpectedConditions.presenceOfAllElementsLocatedBy
                        (By.xpath("//div[@id='gwt-debug-file-structure-mainPanel']//div[@id[contains(.,'gwt-uid-')]]//div[text()]")));
        for (int i = 0; i < items.size(); i++) {
            actionsFactory.createAction(seleniumWebDriver).sendKeys(Keys.ARROW_DOWN).perform();
            WaitUtils.sleepQuietly(1);
            items = new WebDriverWait(seleniumWebDriver, REDRAW_UI_ELEMENTS_TIMEOUT_SEC)
                    .until(ExpectedConditions.presenceOfAllElementsLocatedBy
                            (By.xpath("//div[@id='gwt-debug-file-structure-mainPanel']/div[3]/div/div[1]//div[text()]")));
