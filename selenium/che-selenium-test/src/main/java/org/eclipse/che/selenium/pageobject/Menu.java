...
public class Menu {
    ...
    public void runCommand(String idCommand) {
        new WebDriverWait(seleniumWebDriver, ELEMENT_TIMEOUT_SEC).until(ExpectedConditions.visibilityOfElementLocated(By.id(idCommand)));
     ...
     * @param idCommand
     * @param userDelay
     *         delay for waiting active state menu defined by user
     */
    public void runCommand(String idCommand, int userDelay) {
        new WebDriverWait(seleniumWebDriver, userDelay).until(ExpectedConditions.visibilityOfElementLocated(By.id(idCommand)));
        clickOnCommand(idCommand);
        loader.waitOnClosed();
    }
    ...
    public void runAndWaitCommand(final String idTopMenuCommand, final String idCommandName) {
        (new WebDriverWait(seleniumWebDriver, LOAD_PAGE_TIMEOUT_SEC)).until(new ExpectedCondition<Boolean>() {
            @Override
    ...
    public void runCommand(final String idTopMenuCommand, final String idCommandName) {
        ...
        try {
            new WebDriverWait(seleniumWebDriver,
                              REDRAW_UI_ELEMENTS_TIMEOUT_SEC).until(ExpectedConditions.visibilityOfElementLocated(By.id(idTopMenuCommand)))
                                                             .click();
        ...
        try {
            new WebDriverWait(seleniumWebDriver, ELEMENT_TIMEOUT_SEC).until(ExpectedConditions.visibilityOfElementLocated(By.id(idCommandName)))
                                                                .click();
        } catch (TimeoutException e) {
            seleniumWebDriver.findElement(By.id(idTopMenuCommand)).click();
            new WebDriverWait(seleniumWebDriver, MINIMUM_SEC).until(ExpectedConditions.visibilityOfElementLocated(By.id(idCommandName))).click();
        }
    }
    ...
    public void runCommand(String idTopMenuCommand, String idCommandName, String idSubCommandName) {
        ...
        new WebDriverWait(seleniumWebDriver, REDRAW_UI_ELEMENTS_TIMEOUT_SEC).until(ExpectedConditions.elementToBeClickable(
                By.id(idCommandName)));
        actionsFactory.createAction(seleniumWebDriver).moveToElement(seleniumWebDriver.findElement(By.id(idCommandName))).perform();
        //if element If the element is not drawn in time, in the catch block call submenu again
        try {
            new WebDriverWait(seleniumWebDriver,
                              REDRAW_UI_ELEMENTS_TIMEOUT_SEC).until(ExpectedConditions.visibilityOfElementLocated(By.id(idSubCommandName)));
        } catch (TimeoutException e) {
            LOG.error(e.getLocalizedMessage(), e);
     ...
     */
    public void runCommandByXpath(String idTopMenuCommand, String idCommandName, String xpathSubCommandName) {
        loader.waitOnClosed();
        clickOnCommand(idTopMenuCommand);
        try {
            new WebDriverWait(seleniumWebDriver, ELEMENT_TIMEOUT_SEC).until(ExpectedConditions.visibilityOfElementLocated(By.id(idCommandName)));
    ...
    public void waitCommandIsNotPresentInMenu(String menuCommand) {
        new WebDriverWait(seleniumWebDriver, LOAD_PAGE_TIMEOUT_SEC).until
                (ExpectedConditions.invisibilityOfElementLocated(By.id(menuCommand)));
    }
}
