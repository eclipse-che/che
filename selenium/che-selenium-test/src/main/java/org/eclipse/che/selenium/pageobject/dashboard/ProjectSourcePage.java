...
public class ProjectSourcePage {
    ...
    public enum Sources {
        SAMPLES("Samples"), BLANK("Blank"), GIT("Git"), GITHUB("Github"), ZIP("Zip");
    ...
    @FindBy
            (xpath = Locators.GIT_REPO_XPATH)
    WebElement gitRepositoryInput;

    @FindBy
            (xpath = Locators.ZIP_XPATH)
    WebElement zipLocationInput;

    @FindBy
            (xpath = Locators.ZIP_SKIP_ROOT_XPATH)
    ...
    public void waitOpened() {
        new WebDriverWait(seleniumWebDriver, TestTimeoutsConstants.LOADER_TIMEOUT_SEC).until
                (ExpectedConditions.visibilityOfElementLocated(By.cssSelector(Locators.POPOVER_CSS)));
    }

    public void selectSourceTab(Sources source) {
        WebElement sourceTab = seleniumWebDriver.findElement(By.xpath(String.format(Locators.SOURCE_XPATH, source.title)));
        sourceTab.click();
    }

    public void selectSample(String name) {
        WebElement sample = seleniumWebDriver.findElement(By.xpath(String.format(Locators.SAMPLE_CHECKBOX_XPATH, name)));
        sample.click();
    }
