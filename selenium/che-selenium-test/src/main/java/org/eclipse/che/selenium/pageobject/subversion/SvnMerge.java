...
public class SvnMerge {
    ...
    @Inject
    public SvnMerge(SeleniumWebDriver seleniumWebDriver, Loader loader, ActionsFactory actionsFactory) {
    ...
    public void waitMainFormOpened() {
        new WebDriverWait(seleniumWebDriver, LOAD_PAGE_TIMEOUT_SEC).until(ExpectedConditions.visibilityOf(mainFormMerge));
    }

    public void waitMainFormClosed() {
        new WebDriverWait(seleniumWebDriver, LOAD_PAGE_TIMEOUT_SEC).until(ExpectedConditions.invisibilityOfElementLocated(By.id
                (Locators.MERGE_MAIN_FORM)));
    }
    ...
    public void clickMergeButton() {
        new WebDriverWait(seleniumWebDriver, LOAD_PAGE_TIMEOUT_SEC).until(ExpectedConditions.elementToBeClickable(buttonMerge));
        buttonMerge.click();
    }
    ...
    public void waitItem(String path, int timeout) throws Exception {
        ...
        new WebDriverWait(seleniumWebDriver, timeout).until(ExpectedConditions.presenceOfElementLocated(By.xpath(
                locator)));
    }

    public void openItemByPath(String path) throws Exception {
        String locator = "//span[@id[contains(.,'gwt-debug-projectTree-')] and contains(., '" + path + "')]";
        waitItem(path);
