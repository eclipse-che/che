...
public class NavigateToFile {
    ...
    @Inject
    public NavigateToFile(SeleniumWebDriver seleniumWebDriver, Loader loader, ActionsFactory actionsFactory) {
    ...
    private interface Locators {
        ...
        String FILE_NAME_LIST_SELECT_WITH_PATH = "//div[text()='Enter file name:']//following::tr[contains(., '%s')]";
        String FILE_NAME_LIST_SELECT           = "//div[text()='Enter file name:']/following::table[1]//td[text()='%s']";
    }

    @FindBy
            (id = Locators.NAVI_TO_FILE_FORM)
    WebElement naviToFileForm;

    @FindBy
            (xpath = Locators.FILE_NAME_FIELD)
    WebElement fileNameField;

    @FindBy
            (xpath = Locators.FILE_NAME_POPUP)
    ...
    public void waitFormToClose() {
        new WebDriverWait(seleniumWebDriver, 10).until(ExpectedConditions.invisibilityOfElementLocated(By.id(Locators.NAVI_TO_FILE_FORM)));
    }
    ...
    public void launchNavigateToFileByKeyboard() {
        ...
        action.keyDown(Keys.CONTROL).keyDown(Keys.ALT).sendKeys("n").keyUp(Keys.CONTROL).keyUp(Keys.ALT).perform();
    }
     ...
     *
     * @param symbol
     *         the first symbol of search with key word
     */
     ...
     *
     * @param text
     *         a text that should be into list
     */
    public void waitListOfFilesNames(final String text) {
        new WebDriverWait(seleniumWebDriver, 10).until((ExpectedCondition<Boolean>)
                                                          webDriver -> fileNamePopUp.getText().contains(text));
    }
     ...
     *
     * @param pathName
     *         full name - means the name of file with path
     */
    public void selectFileByFullName(String pathName) {
        new WebDriverWait(seleniumWebDriver, REDRAW_UI_ELEMENTS_TIMEOUT_SEC).until(ExpectedConditions.visibilityOfElementLocated
                (By.xpath(String.format(Locators.FILE_NAME_LIST_SELECT_WITH_PATH, pathName)))).click();
     ...
     *
     * @param nameOfFile
     *         name of a file for searching
     */
    public void selectFileByName(String nameOfFile) {
        new WebDriverWait(seleniumWebDriver, REDRAW_UI_ELEMENTS_TIMEOUT_SEC).until(ExpectedConditions.visibilityOfElementLocated
                (By.xpath(String.format(Locators.FILE_NAME_LIST_SELECT, nameOfFile)))).click();
        actionsFactory.createAction(seleniumWebDriver).doubleClick().perform();
    }
