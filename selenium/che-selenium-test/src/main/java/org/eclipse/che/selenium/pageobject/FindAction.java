...
public class FindAction {
    ...
    private void waitEnterActionFormIsOpen() {
        new WebDriverWait(seleniumWebDriver, LOAD_PAGE_TIMEOUT_SEC).until
                (ExpectedConditions.visibilityOf(enterActionForm));
    }
    ...
    public void typeTextIntoFindActionForm(String text) {
        ...
        for (char symbol : text.toCharArray()) {
            waitWithRedrawTimeout.until(ExpectedConditions.visibilityOf(textBoxActionForm)).sendKeys(Character.toString(symbol));
        }
    ...
    public void clickOnIncludeNonMenuActions() {
        waitWithRedrawTimeout.until(ExpectedConditions.elementToBeClickable(includeNonMenuActions)).click();
    }
    ...
    public void clickOnFoundAction(String action) {
        actionsFactory.createAction(seleniumWebDriver)
             .doubleClick(waitWithRedrawTimeout
                                  .until(ExpectedConditions.elementToBeClickable(By.xpath(String.format(FOUND_ACTION_BY_NAME, action)))))
             .perform();
    ...
    public boolean isCheckBoxSelected() {
        ...
        Object chkBoxObject = js.executeScript("return arguments[0].getAttribute(\"checked\");", includeNonMenuActions);
