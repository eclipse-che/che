...
public class GitAddToIndex {
    interface Locators {
        ...
        String ADD_TO_INDEX_MULTI_FILES = "//button[@id='git-addToIndex-btnAdd']/preceding::textarea[1]";
    }

    @FindBy
            (id = Locators.ADD_TO_INDEX_FORM)
    WebElement form;
    ...
    @FindBy
            (id = Locators.UPDATE_CHKBOX)
    WebElement updateChkbox;

    @FindBy
            (xpath = Locators.ADD_TO_INDEX_MULTI_FILES)
    ...
    public void waitFormToClose() {
        new WebDriverWait(seleniumWebDriver, 7).until(ExpectedConditions.invisibilityOfElementLocated(By.id(Locators.ADD_TO_INDEX_FORM)));
    }
    ...
    public void selectUpdateCheckBox() {
        new WebDriverWait(seleniumWebDriver, LOAD_PAGE_TIMEOUT_SEC).until(
                ExpectedConditions.visibilityOf(updateChkbox)).click();
    }
