...
public class PhpDebugConfig extends AbstractDebugConfig {
    ...
    public void createConfig(String configName, boolean breakAtFirstLine, int debugPort) {
        ...
        new WebDriverWait(seleniumWebDriver, REDRAW_UI_ELEMENTS_TIMEOUT_SEC).until(ExpectedConditions.visibilityOf(configSaveBtn))
                                                                                          .click();
    ...
    private void ensureBreakAtFirstLineCheckboxState(boolean breakAtFirstLine) {
        boolean isSelected = new WebDriverWait(seleniumWebDriver, REDRAW_UI_ELEMENTS_TIMEOUT_SEC)
                .until(ExpectedConditions.presenceOfElementLocated(By.xpath(Locators.BREAK_AT_FIRST_LINE_CHECKBOX_XPATH)))
                .isSelected();
    ...
    void expandDebugCategory() {
        new WebDriverWait(seleniumWebDriver, ATTACHING_ELEM_TO_DOM_SEC).until(ExpectedConditions.visibilityOf(debugCategoryExpandIcon))
                                                                                     .click();
    }
}
