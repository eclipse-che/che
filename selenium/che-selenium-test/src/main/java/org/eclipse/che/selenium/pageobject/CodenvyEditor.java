...
public class CodenvyEditor {
     ...
     *
     * @param startLine
     *         line number of the markers start
     * @param endLine
     *         line number of the markers end
     */
    ...
    /**
     * Check that editor active and focused on first step. After that get cursor position. Note! For correct work a editor must be active.
     *
     * @return char and line position from current visible, active and focused editor (usual uses with split editor)
     */
    public Pair<Integer, Integer> getCursorPositionsFromCurentActive() {
        ...
        return getCursorPositionFromWebElement(
                redrawDriverWait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath(xpathToCurrentActiveCursorPosition))));
    ...
    private Pair<Integer, Integer> getCursorPositionFromWebElement(WebElement webElement) {
        int[] currentCursorPositions =
                Arrays.asList(webElement.getText().split(":")).stream().mapToInt(Integer::parseInt).toArray();
     ...
     *
     * @param lines
     *         List of WebElements that contain text (mostly <div> tags after
