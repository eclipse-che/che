...
public class DebugPanel {
    ...
    @Inject
    public DebugPanel(SeleniumWebDriver seleniumWebDriver, Loader loader, CodenvyEditor editor, CommandsExplorer commandsExplorer) {
    ...
    private interface Locators {
        ...
        String VARIABLE_PANEL_SELECT_VAL     = "//div[@id='gwt-debug-debugger-variablesPanel']//span[text()='%s']";
    }

    interface LocatorsChangeVariable {
        String CHANGE_VARIABLE_BTN = "//button[text()='Change']";
        String CANCEL_BTN          = "//button[text()='Cancel']";
        String TEXTAREA            = "//div[text()='Enter a new value for ']/parent::div/following-sibling::div/textarea";
    }
    ...
    public void selectVarInVariablePanel(String variable) {
        ...
        seleniumWebDriver.findElement(By.xpath(String.format(Locators.VARIABLE_PANEL_SELECT_VAL, variable))).click();
    }

    /**
     * Click on specific button on Debugger panel
     *
     * @param buttonIdLocator
     *         use interface DebuggerButtonsPanel for select buttonIdLocator
     */
    public void clickOnButton(String buttonIdLocator) {
        loader.waitOnClosed();
        new WebDriverWait(seleniumWebDriver, LOAD_PAGE_TIMEOUT_SEC).until(ExpectedConditions.visibilityOfElementLocated(By.id(buttonIdLocator)));
    ...
    public void waitAppearChangeVariableForm() {
        new WebDriverWait(seleniumWebDriver, 20).until(ExpectedConditions.visibilityOf(changeVariableTextAreaForm));
    }
    ...
    public void waitDisappearChangeVariableForm() {
        new WebDriverWait(seleniumWebDriver, 20)
                .until(ExpectedConditions.invisibilityOfElementLocated(By.xpath(LocatorsChangeVariable.TEXTAREA)));
    }
    ...
    public void waitExpectedResultInEvaluateExpression(final String expVal) {
        ...
        new WebDriverWait(seleniumWebDriver, LOAD_PAGE_TIMEOUT_SEC).until((ExpectedCondition<Boolean>)webDriver -> {
            return seleniumWebDriver.findElement(By.xpath(locator)).getAttribute("value").equals(expVal);
        });
    }
    ...
    public void clickEvaluateBtn() {
        ...
        new WebDriverWait(seleniumWebDriver, LOAD_PAGE_TIMEOUT_SEC).until(ExpectedConditions.visibilityOfElementLocated(By.xpath(locator)))
                                                              .click();
    }
    ...
    public void clickCloseEvaluateBtn() {
        ...
        new WebDriverWait(seleniumWebDriver, LOAD_PAGE_TIMEOUT_SEC).until(ExpectedConditions.visibilityOfElementLocated(By.xpath(locator)))
                                                              .click();
        new WebDriverWait(seleniumWebDriver, LOAD_PAGE_TIMEOUT_SEC).until(ExpectedConditions.invisibilityOfElementLocated(By.xpath(locator)));
    ...
    public boolean isDebuggerBtnPanelPresent() {
        ...
        try {
            return seleniumWebDriver.findElement(By.id(DebuggerButtonsPanel.REMOVE_ALL_BREAKPOINTS)).isDisplayed();
        } catch (Exception ex) {
            return false;
        }
    }
     ...
     *
     * @param text
     *         the text under debug - highlighter
     */
    ...
    public void waitBreakPointHighlighterInDefinedPosition(int numOfPosition) {
        ...
        List<WebElement> editorLines =
                seleniumWebDriver.findElements(By.xpath(
                        "//div[@id='gwt-debug-editorPartStack-contentPanel']//div[@active]//div[@class='textviewContent' and @contenteditable='true']/div"));
        new WebDriverWait(seleniumWebDriver, LOAD_PAGE_TIMEOUT_SEC).until((WebDriver driver) -> {
            try {
                return editorLines.get(numOfPosition - 1).findElement(By.tagName("span")).getAttribute("debugid").equals("debug-line");
            } catch (Exception e) {
                return false;
            }
        });
    ...
    /**
     * Click on "Remove All Breakpoints" button in Debug panel and wait until breakpoints go away from Debug panel.
     */
    ...
    /**
     * This method use for stopping Debugger using UI. Check if debugger panel presents. If present click on remove all breakpoints button.
     * than click on stop debugger session button,
     * if some web element covers the panel - browser will be refreshed for closing. If the Debugger have not appeared on UI. After
     * stopping
     * the debugger we call clean up command for tomcat (the command should be prepared in a test. The command stop the tomcat and clea
     * webapp folder)
     *
     * @param cleanUpTomcat
     */
