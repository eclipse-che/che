...
public class MavenPluginStatusBar {
    ...
    @FindBy
            (xpath = Locators.MAVEN_PLUGIN_STATUS_BAR_INFO_XPATH)
    ...
    public String getVisibleTextFromInfoPanel() {
        return new WebDriverWait(seleniumWebDriver, REDRAW_UI_ELEMENTS_TIMEOUT_SEC).until(ExpectedConditions.visibilityOf(infoPanel))
                                                                              .getText();
    }
    ...
    public void waitClosingInfoPanel(final int expectedTimeout) {
        new WebDriverWait(seleniumWebDriver, expectedTimeout).until(ExpectedConditions.invisibilityOfElementLocated(
                By.xpath(Locators.MAVEN_PLUGIN_STATUS_BAR_INFO_XPATH)));
    }
