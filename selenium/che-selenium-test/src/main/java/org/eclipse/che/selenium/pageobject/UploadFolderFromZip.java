...
public class UploadFolderFromZip {
    ...
    @FindBy
            (xpath = Locators.UPLOAD_FOLDER_ZIP_FORM)
    WebElement folderZipUploadForm;

    @FindBy
            (id = Locators.UPLOAD_FOLDER_ZIP_SELECT)
    WebElement folderZipUploadSelect;

    @FindBy
            (id = Locators.UPLOAD_FOLDER_ZIP_OVERWRITE)
    WebElement folderZipUploadOverwrite;

    @FindBy
            (id = Locators.UPLOAD_FOLDER_ZIP_SKIP_ROOT)
    WebElement folderZipUploadSkipRoot;

    @FindBy
            (id = Locators.UPLOAD_FOLDER_ZIP_BUTTON)
    WebElement folderZipUploadButton;

    @FindBy
            (id = Locators.UPLOAD_FOLDER_ZIP_CANCEL)
    WebElement folderZipUploadCancel;

    public void waitUploadFormIsOpen() {
        new WebDriverWait(seleniumWebDriver, 5).until(ExpectedConditions.visibilityOf(folderZipUploadForm));
    }

    public void waitUploadFormIsClosed() {
        new WebDriverWait(seleniumWebDriver, 5).until(ExpectedConditions.invisibilityOfElementLocated
    ...
    public void waitUploadButtonIsDisabled() {
        new WebDriverWait(seleniumWebDriver, 10).until(ExpectedConditions.presenceOfElementLocated
                (By.id(Locators.UPLOAD_FOLDER_ZIP_BUTTON))).getAttribute("disabled");
    }

    public void clickUploadFolderZipButton() {
        new WebDriverWait(seleniumWebDriver, 5).until(ExpectedConditions.elementToBeClickable(folderZipUploadButton));
