...
public class JavaTestRunnerPluginConsole extends Consoles {
    ...
    private static final String TEST_RESULT_TREE_XPATH_TEMPLATE = "//div[contains(@id,'gwt-uid')]//div[text()='%s']";
    private static final String TEST_OUTPUT_XPATH               = "//div[@focused]//div[@id='gwt-debug-commandConsoleLines']//pre";
    private static final String METHODS_MARKED_AS_PASSED        = "//div[contains(@id,'gwt-uid')]//div[@style='color: green;']";
    private static final String METHODS_MARKED_AS_FAILED        = "//div[contains(@id,'gwt-uid')]//div[@style='color: red;']";
     ...
     *
     * @param nameOfFailedMethods
     *         name of that should fail
     */
    ...
    public void waitFqnOfTesClassInResultTree(String fqn) {
        new WebDriverWait(seleniumWebDriver, MINIMUM_SEC)
                .until(ExpectedConditions.visibilityOfAllElementsLocatedBy(By.xpath(String.format(TEST_RESULT_TREE_XPATH_TEMPLATE, fqn))));
    }

    /**
     * get all name of the test methods form the test result tree marked with defined status
     * (may be passed, failed or ignored)
     *
     * @param methodState
     *         the enumeration with defined status
