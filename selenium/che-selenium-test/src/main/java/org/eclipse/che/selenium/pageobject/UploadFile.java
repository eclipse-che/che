...
public class UploadFile {
    ...
    @FindBy
            (xpath = Locators.FILE_UPLOAD_FORM)
    WebElement fileUploadForm;

    @FindBy
            (id = Locators.UPLOAD_FILE_BUTTON)
    WebElement uploadFileButton;

    @FindBy
            (id = Locators.UPLOAD_FILE_CANCEL)
    WebElement uploadFileCancel;

    @FindBy
            (id = Locators.UPLOAD_SELECT_FILE)
    WebElement uploadSelectFile;

    @FindBy
            (id = Locators.UPLOAD_FILE_OVERWRITE)
    ...
    public void waitUploadFormClosed() {
        new WebDriverWait(seleniumWebDriver, 5).until(ExpectedConditions.invisibilityOfElementLocated
                (By.xpath(Locators.FILE_UPLOAD_FORM)));
    }
    ...
    public void waitUploadButtonIsDisabled() {
        new WebDriverWait(seleniumWebDriver, 10).until(ExpectedConditions.presenceOfElementLocated
                (By.id(Locators.UPLOAD_FILE_BUTTON))).getAttribute("disabled");
    }
}
