...
public class CommandsPalette {
    ...
    @Inject
    public CommandsPalette(SeleniumWebDriver seleniumWebDriver, Loader loader, ActionsFactory actionsFactory) {
        this.seleniumWebDriver = seleniumWebDriver;
        this.loader = loader;
        this.actionsFactory = actionsFactory;
        this.redrawUiElementTimeout = new WebDriverWait(seleniumWebDriver, REDRAW_UI_ELEMENTS_TIMEOUT_SEC);
    ...
    private static final class Locators {
        ...
        static final String COMMAND_PALETTE_BUTTON = "//div[@id='gwt-debug-button-open_command_palette']";
        static final String CLOSE_COMMAND_PALETTE  = "//div[contains(text(), 'Commands Palette')]//parent::div//parent::div//div[2]";
        static final String SEARCH_FIELD           = "//input[@id='gwt-debug-commands_palette-filter']";
        static final String COMMANDS_LIST          = "//div[@id = 'gwt-debug-commands_palette']//div[contains(@id,'gwt-uid')]";
        static final String COMMANDS               = "//div[@id = 'gwt-debug-commands_palette']//div[contains(text(),'%s')]";
    }
    ...
    public void openCommandPaletteByHotKeys() {
        loader.waitOnClosed();
        actionsFactory.createAction(seleniumWebDriver).sendKeys(Keys.SHIFT.toString(), Keys.F10.toString()).perform();
        redrawUiElementTimeout.until(ExpectedConditions.visibilityOf(commandPalette));
    }
    ...
    public void closeCommandPalette() {
        ...
        redrawUiElementTimeout.until(ExpectedConditions.invisibilityOfElementLocated(By.xpath(Locators.COMMAND_PALETTE)));
        loader.waitOnClosed();
    }
    ...
    public void searchAndStartCommand(String commandName) {
        redrawUiElementTimeout.until(ExpectedConditions.visibilityOf(searchField)).sendKeys(commandName);
        loader.waitOnClosed();
    }
    ...
    public void commandIsExists(String commandName) {
        redrawUiElementTimeout
                .until(ExpectedConditions.visibilityOfElementLocated(By.xpath(String.format(Locators.COMMANDS, commandName))));
    }

    public void commandIsNotExists(String commandName) {
        redrawUiElementTimeout
                .until(ExpectedConditions.invisibilityOfElementLocated(By.xpath(String.format(Locators.COMMANDS, commandName))));
    }
    ...
    public void startCommandByDoubleClick(String commandName) {
        actions.doubleClick(seleniumWebDriver.findElement(By.xpath(String.format(Locators.COMMANDS, commandName)))).build().perform();
    }
