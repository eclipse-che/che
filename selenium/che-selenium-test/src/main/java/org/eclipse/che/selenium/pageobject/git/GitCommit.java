...
/**
 * Created by aleksandr shmaraaev
 * on 16.09.14.
 */
@Singleton
public class GitCommit {
    ...
    private interface Locators {
        ...
        String BRANCHES_DROPDOWN   = "//span[@id='gwt-debug-push-after-commit-check-box']/following-sibling::select";
        String TREE_ITEM_CHECK_BOX = "//div[@id='gwt-debug-git-compare-changed_files']" +
    ...
    public void waitMainFormCommitIsClosed() {
        new WebDriverWait(seleniumWebDriver, 5).until(ExpectedConditions.invisibilityOfElementLocated(By.id(Locators.MAIN_FORM_COMMIT)));
    }
    ...
    public void selectRemoteBranch(String branch) {
        ...
        new WebDriverWait(seleniumWebDriver, 5).until(ExpectedConditions.elementToBeClickable(
                By.xpath(Locators.BRANCHES_DROPDOWN + String.format("//option[text()='%s']", branch)))).click();
    }
    ...
    public void waitAmendCommitIsSelected() {
        final WebElement element = seleniumWebDriver.findElement(By.id("gwt-debug-git-commit-amend-input"));
        new WebDriverWait(seleniumWebDriver, 5).until((ExpectedCondition<Boolean>)webDriver -> element.getAttribute("checked") != null);
    }
    ...
    public void clickItemCheckBox(String itemName) {
        new WebDriverWait(seleniumWebDriver, REDRAW_UI_ELEMENTS_TIMEOUT_SEC).until(ExpectedConditions.visibilityOfElementLocated
                (By.xpath(String.format(Locators.TREE_ITEM_CHECK_BOX, itemName)))).click();
    ...
    public void waitItemCheckBoxToBeSelected(String itemName) {
        new WebDriverWait(seleniumWebDriver, REDRAW_UI_ELEMENTS_TIMEOUT_SEC).until(ExpectedConditions.elementToBeSelected(
    ...
    public void waitItemCheckBoxToBeUnSelected(String itemName) {
        new WebDriverWait(seleniumWebDriver, REDRAW_UI_ELEMENTS_TIMEOUT_SEC).until(ExpectedConditions.elementSelectionStateToBe(
