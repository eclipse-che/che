...
public class JpaInstallerDao implements InstallerDao {
    ...
    @Inject
    public JpaInstallerDao(@SuppressWarnings("unused") DBInitializer dbInitializer,
    ...
    public long getTotalCount() throws InstallerException {
        try {
            return managerProvider.get().createNamedQuery("Inst.getTotalCount", Long.class).getSingleResult();
        } catch (RuntimeException x) {
            throw new InstallerException(x.getMessage(), x);
        }
    }
