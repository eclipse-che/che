...
@Singleton
public class MachineProcessJsonRpcMessenger implements EventSubscriber<MachineProcessEvent> {
    ...
    @Inject
    public MachineProcessJsonRpcMessenger(RequestTransmitter transmitter, EventService eventService) {
    ...
    public void onEvent(MachineProcessEvent event) {
        ...
        endpointIds.entrySet()
                   ...
                   .forEach(it -> transmitter.transmitOneToNone(it, "event:environment-output:message", event));
    }
    ...
    private void configureSubscribeHandler(RequestHandlerConfigurator configurator) {
        ...
        configurator.newConfiguration()
                    .methodName("event:environment-output:subscribe")
                    .paramsAsString()
                    .noResult()
    ...
    private void configureUnSubscribeHandler(RequestHandlerConfigurator configurator) {
        configurator.newConfiguration()
                    .methodName("event:environment-output:un-subscribe")
                    .paramsAsString()
                    .noResult()
                    .withConsumer((endpointId, workspaceId) -> {
