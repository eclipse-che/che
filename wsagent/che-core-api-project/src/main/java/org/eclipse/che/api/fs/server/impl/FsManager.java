...
@Singleton
public class FsManager implements org.eclipse.che.api.fs.server.FsManager {
  ...
  public File toIoFile(String wsPath) throws NotFoundException {
    ...
    if (file.exists()) {
      ...
      String message = "Cant get IO file for not existing file: " + wsPath;
      LOG.error(message);
  ...
  @Override
  public File toIoFileQuietly(String wsPath) {
    try {
      return toIoFile(wsPath);
