 ...
 * Subclasses specialize this class to generate client-side and server-side implementations.
 *
 * @author Thomas Mäder
 */
public abstract class DtoGenerator {
     ...
     *
     * @param targetFolder
     *         the base folder to generate code to. This is the package folder root.
     * @param targetName
     *         the simple class name to use for the generated class
     * @param targetPackage
     *         the package name to use for the generated class
     * @param sourcePackages
     *         the source packages to use. THe packages must be on the class path at execution time.
     * @throws IOException
     */
    public void generate(File targetFolder, String targetName, String targetPackage, String[] sourcePackages, String[] classNames)
            throws IOException {
        File targetFile = new File(targetFolder,
                                   targetPackage.replace('.', File.separatorChar) + File.separatorChar + targetName + ".java");
        targetFile.getParentFile().mkdirs();
        Set<URL> urls = new HashSet<>();
        for (String pkg : sourcePackages) {
            urls.addAll(ClasspathHelper.forPackage(pkg));
        }
        Reflections reflection = new Reflections(new ConfigurationBuilder().setUrls(urls)
                                                                               ...
                                                                               public boolean apply(String input) {
                                                                                   for (String pkg : sourcePackages) {
                                                                                       if (input.startsWith(pkg)) {
                                                                                           return input.indexOf('.', pkg.length() + 1) ==
                                                                                                  input.lastIndexOf('.');
                                                                                       }
                                                                                   }
                                                                                   return false;
        ...
        try (PrintWriter out = new PrintWriter(targetFile, "utf-8")) {
            ...
            for (Class<? extends Object> clazz : allTypes) {
                out.println(INDENT + INDENT + String.format("dtoFactory.registerProvider(%1$s.class, new %2$s());",
    ...
    private void writeCopyConstructor(String indent, PrintWriter out, Class<? extends Object> clazz, Set<Class<? extends Object>> classes) {
    ...
    private void writeDTOProvider(String indent, PrintWriter out, Class<? extends Object> clazz) {
        String source = indent + "public static class %1$s implements DtoProvider<%2$s> {\n" + indent
