...
public class ToJsonGenerator extends ConversionGenerator {
    ...
    private void generateToJson(String indent, PrintWriter out, String varName, String valueAccess, Type paramType) {
        if (List.class.isAssignableFrom(getRawClass(paramType))) {
            ...
            out.println(String.format(indent + "%1$s %2$s = %3$s;", json.element(), varName,
    ...
    private void generateMapConversion(String indent, PrintWriter out, String varName, String valueAccess,
        ...
        out.println(indent + INDENT + String.format("%1$s.%2$s(%3$s.getKey().toString(), %4$s);", varName, json.put(),
                                                    containedName, varName + "Y"));
        out.println(indent + "}");
    ...
    private String jsonValueExpression(Class<?> paramType, String valueAccess) {
        if (paramType.isPrimitive()) {
            ...
            return String.format("%1$s == null ? %2$s : %3$s;", valueAccess, json.nullValue(),
                                 toJsonConversion(paramType, valueAccess));
        }
    }
    ...
    private class ListConverter {
        ...
        public ListConverter(String varName, String valueAccess,
                             Type paramType) {
