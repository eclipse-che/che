...
---
{{ site.product_formal_name }} is intimately tied to Docker. The {{ site.product_mini_name_capitalized }} server runs as a Docker container and each workspace has a set of environments which may be powered by containers.  Also, our utilities for launching {{ site.product_mini_name_capitalized }}, mounting local IDEs, running smoke tests, and compiling {{ site.product_mini_name_capitalized }} are also built in Docker.

There are numerous best practices that you can follow to get the most out of Docker when using {{ site.product_mini_name_capitalized }}. Also, if your projects depend upon Docker themeslves, there are common questions about Docker in Docker or other similar tactics.
...
3. You want to run {{ site.product_mini_name_capitalized }} in {{ site.product_mini_name_capitalized }}, which allows you to build and run {{ site.product_mini_name_capitalized }} from source code using {{ site.product_mini_name_capitalized }}.

You can make use of Docker from within your workspaces. You need to have a workspace stack that contains a Docker client and you must configure {{ site.product_mini_name_capitalized }} to allow your workspaces to access a Docker daemon,which is usually the one that is already running on your host.

## Docker Daemon Access
There are two ways to configure this.

### Mount Docker Socket
In `che.properties`, add `machine.server.extra.volume=/var/run/docker.sock:/var/run/docker.sock`. This will have your workspace containers perform a volume mount of the Docker socket so that the Docker client in your workspace will use the parent daemon for any Docker CLI commands. If you are using the {{ site.product_mini_name_capitalized }} CLI you can set `CHE_PROPERTY_machine_server_extra_volume=/var/run/docker.sock:/var/run/docker.sock` to achieve the same behavior.  You may need to set up permissions for `/var/run/docker.sock` on the host with `sudo chmod 777 /var/run/docker.sock`.
...
The {{ site.product_mini_name_capitalized }} stack [that ships with {{ site.product_mini_name_capitalized }} is based upon the `docker 1.12.0` image](https://github.com/eclipse/che-dockerfiles/blob/master/recipes/alpine_jdk8/Dockerfile#L9) which includes the latest Docker client within the workspace. You can then make use of docker commands in your workspace by using `sudo docker <command>`.
...
You can also set this using the {{ site.product_mini_name_capitalized }} CLI with an environment variable.
