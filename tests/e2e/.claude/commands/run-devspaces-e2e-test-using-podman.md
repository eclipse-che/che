# Run E2E Tests Against Dev Spaces Using Podman/Container Image

Run E2E tests against Red Hat OpenShift Dev Spaces using the che-e2e container image with Podman. This provides a consistent, isolated test environment with Chrome browser and VNC support.
...
### Basic Dev Spaces Test (with OAuth)
...
podman run -it --shm-size=2g -p 5920:5920 \
  --name selenium-e2e \
  -e TS_SELENIUM_BASE_URL=https://devspaces.apps.ocp.crw-qe.com/ \
...
### Dev Spaces with Local E2E Code Mounted
...
### Running Specific Test Against Dev Spaces
...
### Full Dev Spaces Example with All Common Options (Recommended)
...
podman run -it --shm-size=2g -p 5920:5920 \
  ...
  -e NODE_TLS_REJECT_UNAUTHORIZED=0 \
  -e TESTING_APPLICATION_VERSION=3.23.0 \
...
### Dev Spaces Factory Test with Git Provider OAuth

```bash
podman run -it --shm-size=2g -p 5920:5920 \
  --network="host" \
  --name selenium-e2e \
  -e TS_SELENIUM_BASE_URL=https://devspaces.apps.cluster.example.com \
  -e TS_SELENIUM_OCP_USERNAME=$TS_SELENIUM_OCP_USERNAME \
  -e TS_SELENIUM_OCP_PASSWORD=$TS_SELENIUM_OCP_PASSWORD \
  -e TS_SELENIUM_VALUE_OPENSHIFT_OAUTH=true \
  -e TS_SELENIUM_FACTORY_GIT_REPO_URL=https://github.com/user/repo \
  -e TS_SELENIUM_FACTORY_GIT_PROVIDER=github \
  -e TS_SELENIUM_GIT_PROVIDER_OAUTH=true \
  -e USERSTORY=Factory \
  eclipse/che-e2e:nightly
```

### Dev Spaces on Developer Sandbox

```bash
podman run -it --shm-size=2g -p 5920:5920 \
  --network="host" \
  --name selenium-e2e \
  -e TS_SELENIUM_BASE_URL=https://devspaces.apps.sandbox.x8i5.p1.openshiftapps.com \
  -e TS_SELENIUM_OCP_USERNAME=$TS_SELENIUM_OCP_USERNAME \
  -e TS_SELENIUM_OCP_PASSWORD=$TS_SELENIUM_OCP_PASSWORD \
  -e TS_SELENIUM_VALUE_OPENSHIFT_OAUTH=true \
...
- Always set `TS_SELENIUM_VALUE_OPENSHIFT_OAUTH=true` (Dev Spaces uses OpenShift OAuth)
