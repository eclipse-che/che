...
export class ExtensionsView {
	...
	async openExtensionsView(): Promise<void> {
		...
		const extensionsViewVisible: boolean = await this.driverHelper.waitVisibilityBoolean(
			ExtensionsView.EXTENSIONS_VIEW,
			TIMEOUT_CONSTANTS.TS_SELENIUM_DEFAULT_ATTEMPTS,
	...
	/**
	 * open the More Actions menu in Extensions view
	 */
	 ...
	 * Uses @installed filter to find them
	 *
