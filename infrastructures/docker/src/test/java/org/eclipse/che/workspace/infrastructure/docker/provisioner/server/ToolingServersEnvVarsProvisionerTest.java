...
public class ToolingServersEnvVarsProvisionerTest {
    private static final RuntimeIdentity RUNTIME_IDENTITY = new RuntimeIdentityImpl("testWsId", "testEnv", "testOwner");
    private static final String          CONTAINER_1_NAME = "cont1";
    private static final String          CONTAINER_2_NAME = "cont2";

    @Mock
    private ServerEnvironmentVariableProvider provider1;
    @Mock
    private ServerEnvironmentVariableProvider provider2;
    @Mock
    ...
    public void shouldProvideRuntimeIdentityToEnvProviders() throws Exception {
        // given
        ToolingServersEnvVarsProvisioner provisioner = new ToolingServersEnvVarsProvisioner(singleton(provider1));
    ...
    public void shouldIgnoreNullResultsFromProviders() throws Exception {
        ...
        DockerEnvironment expected = new DockerEnvironment()
                .setContainers(singletonMap(CONTAINER_1_NAME,
    ...
    public void shouldNotRemoveExistingEnvVarsWithDifferentNames() throws Exception {
        ...
        when(provider1.get(any(RuntimeIdentity.class))).thenReturn(envVar1);
        dockerEnvironment.getContainers().get(CONTAINER_1_NAME).getEnvironment()
                         .put(existingEnvVar.first, existingEnvVar.second);
        DockerEnvironment expected = new DockerEnvironment();
        expected.getContainers().put(CONTAINER_1_NAME, new DockerContainerConfig().setEnvironment(
                ImmutableMap.of(envVar1.first, envVar1.second, existingEnvVar.first, existingEnvVar.second)));
    ...
    public void shouldReplaceExistingEnvVarsWithMatchingNames() throws Exception {
        ...
        when(provider1.get(any(RuntimeIdentity.class))).thenReturn(Pair.of(existingEnvVarName,
                                                                           envVarValueFromProvider));
        dockerEnvironment.getContainers().get(CONTAINER_1_NAME).getEnvironment()
                         .put(existingEnvVarName, oldEnvVarValue);
        DockerEnvironment expected = new DockerEnvironment();
        expected.getContainers().put(CONTAINER_1_NAME, new DockerContainerConfig().setEnvironment(
                singletonMap(existingEnvVarName, envVarValueFromProvider)));
