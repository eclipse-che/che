...
public class CpuLimitsProvisionerTest {
    ...
    public void shouldSetCPULimitsInBuildArgs() throws Exception {
        // given
        dockerEnvironment.getContainers().put("cont1", new DockerContainerConfig().setBuild(new DockerBuildContext()));
        dockerEnvironment.getContainers().put("cont2", new DockerContainerConfig());
        DockerEnvironment expected = new DockerEnvironment();
        expected.getContainers().put("cont1", new DockerContainerConfig().setCpuPeriod(CPU_PERIOD)
                                                                         .setCpuQuota(CPU_QUOTA)
                                                                         .setCpuSet(CPU_SET)
                                                                         .setBuild(new DockerBuildContext()
