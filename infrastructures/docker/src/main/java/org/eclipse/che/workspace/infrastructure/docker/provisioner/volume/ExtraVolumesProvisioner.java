...
/**
 * Adds extra volumes to {@link DockerContainerConfig}.
 *
 * </p>Provides volume configuration of machine for any local directories
 * that a user may want to mount into any docker machine.
 * </br>{@code che.workspace.volume} property is optional
...
public class ExtraVolumesProvisioner implements ContainerSystemSettingsProvisioner {
    ...
    public ExtraVolumesProvisioner(@Named("che.workspace.volume") String volume) {
        if (isNullOrEmpty(volume)) {
            ...
            this.extraVolumes = Arrays.stream(volume.split(";"))
                                      .filter(s -> !s.isEmpty())
                                      .collect(toSet());
        }
    }
