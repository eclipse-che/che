...
public class OpenShiftMachine implements Machine {
    ...
    public OpenShiftMachine(String podName,
    ...
    void setStatus(String serverRef, ServerStatus status) {
        ...
        if (server == null) {
            throw new IllegalArgumentException("Server with provided reference " + serverRef + " is missed");
        }
    ...
    public void waitRunning(int timeoutMin) throws InfrastructureException {
        project.pods().wait(podName, timeoutMin, p -> (OPENSHIFT_POD_STATUS_RUNNING.equals(p.getStatus().getPhase())));
    }
}
