...
public class ScheduleInjectionListener<T> extends LifecycleModule implements InjectionListener<T> {
    ...
    private final Matcher<AnnotatedElement> javaxSingleton  = Matchers.annotatedWith(javax.inject.Singleton.class);
    private final Matcher<AnnotatedElement> googleSingleton = Matchers.annotatedWith(com.google.inject.Singleton.class);

    private final Provider<Launcher> launcherProvider;
    private final Provider<Injector> injectorProvider;

    public ScheduleInjectionListener(Provider<Launcher> launcherProvider, Provider<Injector> injectorProvider) {
    ...
    private <T> T getValue(Class<T> configurationType, String configurationKey) {
        try {
            return injectorProvider.get().getInstance(Key.get(configurationType, Names.named(configurationKey)));
    ...
    private void launch(Object object, Method method, ScheduleDelay annotation) {
        ...
        launcher.scheduleWithFixedDelay(new LoggedRunnable(object,
                                        ...
                                        annotation.delayParameterName().isEmpty() ? annotation.delay()
                                                                                  : getValue(
                                                                                          annotation.delayParameterName()),
                                        annotation.unit());
    }
