...
/**
 * Rate exceed detector. It used to detect exceeding of rate of some operation. If rate is exceeded then method {@link
 * #updateAndCheckRate()} returns {@code true}. A {@code RateExceedDetector} gets max allowed permits per seconds in constructor, then it
 * checks that required time have elapsed between two calls of method {@link #updateAndCheckRate()}.
 * <p/>
 * Implementation is not threadsafe and required external synchronization if it's used in multi-thread environment.
 * An example that prints on stdout if rate exceed 5 per seconds limit:
 * <pre>
 * {@code
