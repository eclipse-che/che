...
let Editor = Editor_1 = class Editor {
    ...
    activateBreakpoint(tabTitle, lineNumber, timeout = TestConstants_1.TestConstants.TS_SELENIUM_DEFAULT_TIMEOUT) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            for (let i = 0; i < attempts; i++) {
                const elementLocator = yield this.getLineNumberBlockLocator(tabTitle, lineNumber);
                const element = yield this.driverHelper.waitVisibility(elementLocator, timeout);
                try {
                    yield this.driverHelper.getAction().mouseMove(element, { x: 5, y: 5 }).perform();
                    yield this.waitBreakpointHint(tabTitle, lineNumber);
                    yield this.driverHelper.getAction().click().perform();
    ...
    getLineNumberBlockLocator(tabTitle, lineNumber) {
        return __awaiter(this, void 0, void 0, function* () {
            const lineYPixelCoordinates = yield this.getLineYCoordinates(lineNumber);
            return selenium_webdriver_1.By.xpath(`//div[contains(@id, '${tabTitle}')]//div[@class='margin']` +
                `//div[contains(@style, '${lineYPixelCoordinates}px')]`);
    ...
    getSuggestionLineXpathLocator(suggestionText) {
        return selenium_webdriver_1.By.xpath(`//div[@widgetid='editor.widget.suggestWidget']//div[@aria-label='${suggestionText}, suggestion, has details']`);
    }
