...
public class SignalEventUtils {
  ...
  public static SignalEvent create(
      com.google.gwt.user.client.Event rawEvent, boolean cancelBubbleIfNullified) {
    return SignalEventImpl.create(rawEvent, cancelBubbleIfNullified);
  }

  /**
   * Returns the paste contents from a "paste" event, or null if it is not a paste event or cannot
   * be retrieved.
   */
