...
/**
 * This is an import of the unicode character categories table. It can be used to create {@link
 * RegExp} which match based on the type of character (something not available easily via the
 ...
 * <h3>How-To</h3>
 *
 * This utility class is full of strings consisting of valid regular expression character ranges
 * that identify the character classes in Unicode 6.0. The theory is pretty simple, pick the
 * categories you are interested in and create a regular expression by using {@link
 * CharacterClass#create(String...)} to create a character class which matches the provided groups
 * or {@link CharacterClass#invert(String...)} which matches any characters but the provided groups.
 * Once created, pass the {@link CharacterClass} into {@link #compile(CharacterClass...)} to compile
 * them together into a single regular expression. By compiling two {@link CharacterClass} into a
 * single regex you can exclude certain characters from a group (an example being {@link
...
public class UnicodeUtils {
  ...
  private static final String lowercaseRegexp =
  ...
  private static final String uppercaseRegexp =
  ...
  private static final String titlecaseLetterRegexp =
      "\u01C5\u01C8\u01CB\u01F2\u1F88-\u1F8F\u1F98-\u1F9F\u1FA8-\u1FAF\u1FBC\u1FCC\u1FFC";

  private static final String modifierLetterRegexp =
      "\u02B0-\u02C1\u02C6-\u02D1\u02E0-\u02E4\u02EC\u02EE\u0374\u037A\u0559\u0640\u06E5-\u06E6\u07F4-\u07F5\u07FA\u081A\u0824"
          + "\u0828\u0971\u0E46\u0EC6\u10FC\u17D7\u1843\u1AA7\u1C78-\u1C7D\u1D2C-\u1D61\u1D78\u1D9B-\u1DBF\u2071\u207F\u2090-\u209C\u2C7D"
          + "\u2D6F\u2E2F\u3005\u3031-\u3035\u303B\u309D-\u309E\u30FC-\u30FE\uA015\uA4F8-\uA4FD\uA60C\uA67F\uA717-\uA71F\uA770\uA788"
          + "\uA9CF\uAA70\uAADD\uFF70\uFF9E-\uFF9F";

  private static final String letterWithoutCaseRegexp =
  ...
  private static final String nonSpacingMarkRegexp =
  ...
  private static final String spacingCombiningMarkRegexp =
  ...
  private static final String enclosingMarkRegexp =
      "\u0488-\u0489\u20DD-\u20E0\u20E2-\u20E4\uA670-\uA672";

  private static final String decimalDigitRegexp =
      "\u0030-\u0039\u0660-\u0669\u06F0-\u06F9\u07C0-\u07C9\u0966-\u096F\u09E6-\u09EF\u0A66-\u0A6F\u0AE6-\u0AEF\u0B66-\u0B6F\u0BE6"
          + "-\u0BEF\u0C66-\u0C6F\u0CE6-\u0CEF\u0D66-\u0D6F\u0E50-\u0E59\u0ED0-\u0ED9\u0F20-\u0F29\u1040-\u1049\u1090-\u1099\u17E0-\u17E9"
          + "\u1810-\u1819\u1946-\u194F\u19D0-\u19D9\u1A80-\u1A89\u1A90-\u1A99\u1B50-\u1B59\u1BB0-\u1BB9\u1C40-\u1C49\u1C50-\u1C59\uA620"
          + "-\uA629\uA8D0-\uA8D9\uA900-\uA909\uA9D0-\uA9D9\uAA50-\uAA59\uABF0-\uABF9\uFF10-\uFF19";

  private static final String letterNumberRegexp =
      "\u16EE-\u16F0\u2160-\u2182\u2185-\u2188\u3007\u3021-\u3029\u3038-\u303A\uA6E6-\uA6EF";

  private static final String otherNumberRegexp =
      "\u00B2-\u00B3\u00B9\u00BC-\u00BE\u09F4-\u09F9\u0B72-\u0B77\u0BF0-\u0BF2\u0C78-\u0C7E\u0D70-\u0D75\u0F2A-\u0F33\u1369-\u137C"
          + "\u17F0-\u17F9\u19DA\u2070\u2074-\u2079\u2080-\u2089\u2150-\u215F\u2189\u2460-\u249B\u24EA-\u24FF\u2776-\u2793\u2CFD\u3192"
          + "-\u3195\u3220-\u3229\u3251-\u325F\u3280-\u3289\u32B1-\u32BF\uA830-\uA835";

  private static final String dashPunctuationRegexp =
      "\u002D\u058A\u05BE\u1400\u1806\u2010-\u2015\u2E17\u2E1A\u301C\u3030\u30A0\uFE31-\uFE32\uFE58\uFE63\uFF0D";

  private static final String openPuncutationRegexp =
      "\u0028\u005B\u007B\u0F3A\u0F3C\u169B\u201A\u201E\u2045\u207D\u208D\u2329\u2768\u276A\u276C\u276E\u2770\u2772\u2774\u27C5"
          + "\u27E6\u27E8\u27EA\u27EC\u27EE\u2983\u2985\u2987\u2989\u298B\u298D\u298F\u2991\u2993\u2995\u2997\u29D8\u29DA\u29FC\u2E22"
          + "\u2E24\u2E26\u2E28\u3008\u300A\u300C\u300E\u3010\u3014\u3016\u3018\u301A\u301D\uFD3E\uFE17\uFE35\uFE37\uFE39\uFE3B\uFE3D"
          + "\uFE3F\uFE41\uFE43\uFE47\uFE59\uFE5B\uFE5D\uFF08\uFF3B\uFF5B\uFF5F\uFF62";

  private static final String closePuncutationRegexp =
      "\u0029\u005D\u007D\u0F3B\u0F3D\u169C\u2046\u207E\u208E\u232A\u2769\u276B\u276D\u276F\u2771\u2773\u2775\u27C6\u27E7\u27E9"
          + "\u27EB\u27ED\u27EF\u2984\u2986\u2988\u298A\u298C\u298E\u2990\u2992\u2994\u2996\u2998\u29D9\u29DB\u29FD\u2E23\u2E25\u2E27"
          + "\u2E29\u3009\u300B\u300D\u300F\u3011\u3015\u3017\u3019\u301B\u301E-\u301F\uFD3F\uFE18\uFE36\uFE38\uFE3A\uFE3C\uFE3E\uFE40"
          + "\uFE42\uFE44\uFE48\uFE5A\uFE5C\uFE5E\uFF09\uFF3D\uFF5D\uFF60\uFF63";

  private static final String initialPunctuationRegexp =
      "\u00AB\u2018\u201B-\u201C\u201F\u2039\u2E02\u2E04\u2E09\u2E0C\u2E1C\u2E20";

  private static final String finalPuncutationRegexp =
      "\u00BB\u2019\u201D\u203A\u2E03\u2E05\u2E0A\u2E0D\u2E1D\u2E21";

  private static final String connectorPuncutationRegexp =
      "\u005F\u203F-\u2040\u2054\uFE33-\uFE34\uFE4D-\uFE4F\uFF3F";

  /* \u005C is a '\' we have to repeat it twice to escape itself! */
  private static final String punctuationOtherRegexp =
      "\u0021-\u0023\u0025-\u0027\u002A\u002C\u002E-\u002F\u003A-\u003B\u003F-\u0040\u005C\u005C\u00A1\u00B7\u00BF\u037E\u0387"
          + "\u055A-\u055F\u0589\u05C0\u05C3\u05C6\u05F3-\u05F4\u0609-\u060A\u060C-\u060D\u061B\u061E-\u061F\u066A-\u066D\u06D4\u0700"
          + "-\u070D\u07F7-\u07F9\u0830-\u083E\u085E\u0964-\u0965\u0970\u0DF4\u0E4F\u0E5A-\u0E5B\u0F04-\u0F12\u0F85\u0FD0-\u0FD4\u0FD9"
          + "-\u0FDA\u104A-\u104F\u10FB\u1361-\u1368\u166D-\u166E\u16EB-\u16ED\u1735-\u1736\u17D4-\u17D6\u17D8-\u17DA\u1800-\u1805\u1807"
  ...
  private static final String mathSymbolRegexp =
      "\u002B\u003C-\u003E\u007C\u007E\u00AC\u00B1\u00D7\u00F7\u03F6\u0606-\u0608\u2044\u2052\u207A-\u207C\u208A-\u208C\u2118\u2140"
          + "-\u2144\u214B\u2190-\u2194\u219A-\u219B\u21A0\u21A3\u21A6\u21AE\u21CE-\u21CF\u21D2\u21D4\u21F4-\u22FF\u2308-\u230B\u2320"
          + "-\u2321\u237C\u239B-\u23B3\u23DC-\u23E1\u25B7\u25C1\u25F8-\u25FF\u266F\u27C0-\u27C4\u27C7-\u27CA\u27CC\u27CE-\u27E5\u27F0"
          + "-\u27FF\u2900-\u2982\u2999-\u29D7\u29DC-\u29FB\u29FE-\u2AFF\u2B30-\u2B44\u2B47-\u2B4C\uFB29\uFE62\uFE64-\uFE66\uFF0B\uFF1C"
  ...
  private static final String currencySymbolRegexp =
      "\u0024\u00A2-\u00A5\u060B\u09F2-\u09F3\u09FB\u0AF1\u0BF9\u0E3F\u17DB\u20A0-\u20B9\uA838\uFDFC\uFE69\uFF04\uFFE0-\uFFE1\uFFE5"
          + "-\uFFE6";

  private static final String modifierSymbolRegexp =
      "\u005E\u0060\u00A8\u00AF\u00B4\u00B8\u02C2-\u02C5\u02D2-\u02DF\u02E5-\u02EB\u02ED\u02EF-\u02FF\u0375\u0384-\u0385\u1FBD"
          + "\u1FBF-\u1FC1\u1FCD-\u1FCF\u1FDD-\u1FDF\u1FED-\u1FEF\u1FFD-\u1FFE\u309B-\u309C\uA700-\uA716\uA720-\uA721\uA789-\uA78A\uFBB2"
          + "-\uFBC1\uFF3E\uFF40\uFFE3";

  private static final String otherSymbolRegexp =
      "\u00A6-\u00A7\u00A9\u00AE\u00B0\u00B6\u0482\u060E-\u060F\u06DE\u06E9\u06FD-\u06FE\u07F6\u09FA\u0B70\u0BF3-\u0BF8\u0BFA\u0C7F"
          + "\u0D79\u0F01-\u0F03\u0F13-\u0F17\u0F1A-\u0F1F\u0F34\u0F36\u0F38\u0FBE-\u0FC5\u0FC7-\u0FCC\u0FCE-\u0FCF\u0FD5-\u0FD8\u109E"
          + "-\u109F\u1360\u1390-\u1399\u1940\u19DE-\u19FF\u1B61-\u1B6A\u1B74-\u1B7C\u2100-\u2101\u2103-\u2106\u2108-\u2109\u2114\u2116"
          + "-\u2117\u211E-\u2123\u2125\u2127\u2129\u212E\u213A-\u213B\u214A\u214C-\u214D\u214F\u2195-\u2199\u219C-\u219F\u21A1-\u21A2"
  ...
  private static final String spaceSeparatorRegexp =
      "\u0020\u00A0\u1680\u180E\u2000-\u200A\u202F\u205F\u3000";

  private static final String lineSeparatorRegexp = "\u2028";

  private static final String paragraphSeparatorRegexp = "\u2029";

  private static final String otherControlRegexp = "\u0000-\u001F\u007F-\u009F";

  private static final String otherFormatRegexp =
      "\u00AD\u0600-\u0603\u06DD\u070F\u17B4-\u17B5\u200B-\u200F\u202A-\u202E\u2060-\u2064\u206A-\u206F\uFEFF\uFFF9-\uFFFB";

  private static final String otherPrivateRegexp = "\uE000-\uF8FF";

  private static final String otherSurrogateRegexp = "\uD800-\uDFFF";

  private static final String otherUnassignedRegexp =
      "\u0378\u0379\u037F-\u0383\u038B\u038D\u03A2\u0526-\u0530\u0557\u0558\u0560\u0588\u058B-\u0590\u05C8-\u05CF\u05EB-\u05EF"
          + "\u05F5-\u05FF\u0604\u0605\u061C\u061D\u0620\u065F\u070E\u074B\u074C\u07B2-\u07BF\u07FB-\u07FF\u082E\u082F\u083F-\u08FF\u093A"
          + "\u093B\u094F\u0956\u0957\u0973-\u0978\u0980\u0984\u098D\u098E\u0991\u0992\u09A9\u09B1\u09B3-\u09B5\u09BA\u09BB\u09C5\u09C6"
          + "\u09C9\u09CA\u09CF-\u09D6\u09D8-\u09DB\u09DE\u09E4\u09E5\u09FC-\u0A00\u0A04\u0A0B-\u0A0E\u0A11\u0A12\u0A29\u0A31\u0A34\u0A37"
  ...
  private static final String anyLetterRegexp =
      join(
          lowercaseRegexp,
          uppercaseRegexp,
  ...
  private static final String anyMarkRegexp =
      join(nonSpacingMarkRegexp, spacingCombiningMarkRegexp, enclosingMarkRegexp);

  private static final String anyNumberRegexp =
      join(decimalDigitRegexp, letterNumberRegexp, otherNumberRegexp);

  private static final String anyPuncutationRegexp =
      join(
          dashPunctuationRegexp,
  ...
  private static final String anySymbolRegexp =
      join(mathSymbolRegexp, currencySymbolRegexp, modifierSymbolRegexp, otherSymbolRegexp);

  private static final String anySeparatorRegExp =
      join(spaceSeparatorRegexp, lineSeparatorRegexp, paragraphSeparatorRegexp);

  private static final String anyOtherRegexp =
      join(
          otherControlRegexp,
          otherFormatRegexp,
          otherPrivateRegexp,
          otherSurrogateRegexp,
          otherUnassignedRegexp);

  // Custom Categories
  private static final String unicodeWhitespaceRegexp =
      join(
          spaceSeparatorRegexp,
          lineSeparatorRegexp,
  ...
  private static final String javascriptIdentifierRegexp =
      join(
          nonSpacingMarkRegexp,
  ...
  private static final String nonAsciiCharacter = join("\u0100-\uFFFF");

  // Custom Regular Expressions
  /** Matches any character which is not a Unicode mark. */
  public static final RegExp regexpNotMark = compile(CharacterClass.invert(anyMarkRegexp));

  /**
   * Matches any character which is not a Unicode mark or other character excluding the tab
   * character.
   */
  public static final RegExp regexpNotMarkOrOtherExcludingTabAndNewline =
      compile(CharacterClass.invert(anyMarkRegexp, anyOtherRegexp), CharacterClass.create("\t\n"));

  /**
   * Matches any non-ascii character, tab, or carriage return character. Also matches any marks
   * following the character in a separate capturing group.
   */
  public static final RegExp regexpNonAsciiTabOrCarriageReturn =
      compile(
          CharacterClass.concat(
              CharacterClass.create(nonAsciiCharacter, "\r\t"),
              CharacterClass.Builder.create()
                  .setCapturing(true)
                  .setOccurences(CharacterClass.Builder.Occurences.ZERO_OR_MORE)
                  .build(anyMarkRegexp, "\r")));

  /**
   * The official definition of whitespace according to unicode 6.0. Includes all separator type
   * characters, + NEL, tab, line feed and carriage return.
   */
  public static final RegExp regexpNotWhitespace =
      compile(CharacterClass.invert(unicodeWhitespaceRegexp));

  /** Valid character for a javascript identifier. */
  public static final RegExp regexpNotJavascriptIdentifierCharacter =
      compile(CharacterClass.invert(javascriptIdentifierRegexp));

  /** Not a valid javascript identifier character or whitespace. */
  public static final RegExp regexpIdentifierOrWhitespace =
      compile(CharacterClass.create(unicodeWhitespaceRegexp, javascriptIdentifierRegexp));

  /**
   * The official definition of whitespace according to unicode 6.0 excluding \r and \n control
   * characters.
   */
  public static final RegExp regexpNotWhitespaceExcludingNewlineAndCarriageReturn =
  ...
  /**
   * Returns true if a chracter is whitespace according to Unicode 6.0 (excluding the characters \r
   * and \n).
   */
  ...
  /**
   * Simple helper class for strongly typed character classes. Just helps to prevent typos and makes
   * it more clear on the intent of the creator.
   */
  private static class CharacterClass {
    ...
    /**
     * Creates a character class which can be inverted, capturing and occur multiple times.
     * Capturing takes precedent over occurrences so if a character class is capturing the number of
     * occurrences will apply to the entire capturing group.
     */
    public static class Builder {
      ...
      private static final Builder INVERTED_CLASS = Builder.create().setInverted(true);

      private static final Builder NORMAL_CLASS = Builder.create();
  ...
  /**
   * Joins character classes by OR'ing them together then compiles them into a single regular
   * expression.
   */
