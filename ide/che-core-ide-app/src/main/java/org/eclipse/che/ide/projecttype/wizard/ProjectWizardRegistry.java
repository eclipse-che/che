...
public class ProjectWizardRegistry {
    ...
    private void register(Set<ProjectWizardRegistrar> registrars) {
        for (ProjectWizardRegistrar registrar : registrars) {
            ...
            if (this.registrars.containsKey(id)) {
                Log.warn(ProjectWizardRegistry.class, "Wizard for project type " + id + " already registered.");
            } else {
     ...
     * @return {@link ProjectWizardRegistrar} for the specified project type ID or {@code Optional#empty()} if none
     */
