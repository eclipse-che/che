...
public class PanelSelectorViewImpl extends Composite implements PanelSelectorView {
    ...
    @UiField
    FlowPanel selectorLeft;

    @UiField
    FlowPanel selectorLeftBottom;

    @UiField
    FlowPanel selectorFullEditor;

    @UiField
    FlowPanel selectorBottom;

    @UiField
    FlowPanel selectorRight;

    @UiField
    FlowPanel selectorLeftRightBottom;
    ...
    @Inject
    public PanelSelectorViewImpl(PanelSelectorViewImplUiBinder uiBinder,
                                 Resources resources) {
    ...
    public void showPopup() {
        ...
        clickListenerRemover = Elements.getBody().addEventListener(Event.MOUSEDOWN, clickListener, true);
    }
    ...
    private final EventListener clickListener = new EventListener() {
        ...
        public void handleEvent(Event evt) {
            if (((Element)popup.getElement()).contains((Node)evt.getTarget())) {
                ...
                } else if (((Element)selectorLeftBottom.getElement()).contains((Node)evt.getTarget())) {
                    delegate.onSelectorLeftBottomClicked();
                } else if (((Element)selectorFullEditor.getElement()).contains((Node)evt.getTarget())) {
                ...
                } else if (((Element)selectorLeftRightBottom.getElement()).contains((Node)evt.getTarget())) {
