...
public class ProjectTemplateRegistryImpl implements ProjectTemplateRegistry {
    ...
    @Inject
    ProjectTemplateRegistryImpl(DtoUnmarshallerFactory dtoUnmarshallerFactory,
    ...
    private void registerAllTemplates() {
        fetchTemplates().then(templateDescriptors -> {
            templateDescriptors.forEach(this::register);
        }).catchError(error -> {
            Log.error(ProjectTemplateRegistryImpl.this.getClass(), "Can't load project templates: " + error.getCause());
        });
    }

    private void register(ProjectTemplateDescriptor descriptor) {
        templateDescriptors.computeIfAbsent(descriptor.getProjectType(), key -> new ArrayList<>())
                           .add(descriptor);
    }
