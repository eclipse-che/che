...
public class FormatterPreferencePagePresenter extends AbstractPreferencePagePresenter
    ...
    @Inject
    public FormatterPreferencePagePresenter(CoreLocalizationConstant localizationConstant,
    ...
    public void onImportButtonClicked() {
        ...
        final StatusNotification notification = new StatusNotification("Importing Formatter...", PROGRESS, FLOAT_MODE);
        notificationManager.notify(notification);

        String fileContent = view.getFileContent();
        projectServiceClient.getItem(filePath).then(arg -> {
    ...
    private void createFormatter(Path filePath, String fileContent, StatusNotification notification) {
        projectServiceClient.createFile(filePath, fileContent).then(arg -> {
    ...
    private void updateFormatter(Path filePath, String fileContent, StatusNotification notification) {
        projectServiceClient.setFileContent(filePath, fileContent).then(arg -> {
            notification.setStatus(SUCCESS);
