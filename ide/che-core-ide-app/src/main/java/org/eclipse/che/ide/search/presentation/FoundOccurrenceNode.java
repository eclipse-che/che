...
/**
 * Instance describe occurrence where found give text.
 * If in file found several occurrence, will be created dedicated instance for each occurrence
 *
...
public class FoundOccurrenceNode extends AbstractTreeNode implements HasPresentation, HasAction {
    ...
    public void actionPerformed() {
        final EditorPartPresenter editorPartPresenter = editorAgent.getOpenedEditor(Path.valueOf(itemPath));
        ...
        appContext.getWorkspaceRoot().getFile(itemPath).then(file -> {
            if (file.isPresent()) {
