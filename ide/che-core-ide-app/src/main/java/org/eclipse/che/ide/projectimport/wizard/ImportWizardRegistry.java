...
public class ImportWizardRegistry {
    ...
    private void register(Set<ImportWizardRegistrar> registrars) {
        for (ImportWizardRegistrar registrar : registrars) {
            ...
            if (this.registrars.containsKey(id)) {
                Log.warn(ImportWizardRegistry.class, "Wizard for project importer " + id + " already registered.");
            } else {
     ...
     * @return {@link ImportWizardRegistrar} for the specified project importer ID or {@code Optional#empty()} if none
     */
