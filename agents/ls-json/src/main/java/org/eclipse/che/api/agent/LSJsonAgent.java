 ...
 */
@Singleton
public class LSJsonAgent implements Agent {
    ...
    private InputStream readResource(String name) throws IOException {
        InputStream inputStream = LSJsonAgent.class.getResourceAsStream("/" + name);
        if (inputStream == null) {
            throw new IOException(format("Can't initialize Language server json agent. Resource %s not found", name));
        }
        return inputStream;
    }
}
