 ...
 */
@Singleton
public class UnisonAgent implements Agent {
    ...
    private InputStream readResource(String name) throws IOException {
        InputStream inputStream = UnisonAgent.class.getResourceAsStream("/" + name);
        if (inputStream == null) {
            throw new IOException(format("Can't initialize unison agent. Resource %s not found", name));
        }
        return inputStream;
    }
}
