...
const (
	// ConnectedEventType is type of event that is sent to a client when it connects to server
...
var (
	...
	HTTPRoutes = rest.RoutesGroup{
		...
		Items: []rest.Route{
			{
				Method:     "GET",
				Name:       "Connect to Exec-Agent(webscoket)",
				Path:       "/connect",
				HandleFunc: registerChannel,
			},
		},
	}
...
// Channel describes channel which is websocket connection
...
// WsMessage represents struct for reading raw websocket messages
type WsMessage struct {
	err   error
	bytes []byte
}

// MessageHandler handles raw messages received from websocket channel
...
// GetChannel gets channel by the channel id, if there is no such channel
...
// GetChannels returns all the currently registered channels.
...
// DropChannel drops the channel with the given id.
