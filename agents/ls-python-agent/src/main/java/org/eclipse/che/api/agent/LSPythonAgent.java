 ...
 */
@Singleton
public class LSPythonAgent implements Agent {
    ...
    private InputStream readResource(String name) throws IOException {
        InputStream inputStream = LSPythonAgent.class.getResourceAsStream("/" + name);
        if (inputStream == null) {
            throw new IOException(format("Can't initialize Language server python agent. Resource %s not found", name));
        }
        return inputStream;
    }
}
