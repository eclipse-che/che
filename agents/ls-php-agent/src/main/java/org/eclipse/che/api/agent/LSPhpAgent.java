 ...
 */
@Singleton
public class LSPhpAgent implements Agent {
    ...
    private InputStream readResource(String name) throws IOException {
        InputStream inputStream = LSPhpAgent.class.getResourceAsStream("/" + name);
        if (inputStream == null) {
            throw new IOException(format("Can't initialize Language server php agent. Resource %s not found", name));
        }
        return inputStream;
    }
}
